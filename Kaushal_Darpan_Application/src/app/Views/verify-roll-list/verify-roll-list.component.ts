...
@Component({
  selector: 'app-verify-roll-list',
  standalone: false,
  templateUrl: './verify-roll-list.component.html',
  styleUrl: './verify-roll-list.component.css'
})
export class VerifyRollListComponent {
...
/*      this.SearchReqForEnroll.ModuleID = 2;*/
  ...
  ForwardRollNo() {
    ...
    this.Swal2.Confirmation("Are you sure you want to forward the Roll list for verification?", async (result: any) => {
  ...
  async SaveApplicationWorkFlow(Action: number) {
    ...
    try
    {
      ...
      if (this.sSOLoginDataModel.RoleID == EnumRole.Registrar || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge_NonEng || this.sSOLoginDataModel.RoleID == EnumRole.Registrar_NonEng) {
        Selected.forEach(e => {
          if (e.IsRegistrarVerified == true && e.Status == EnumEnrollNoStatus.Verified) {
