...
import { CenterAllocationSearchModel, CenterAllocationtDataModels, CenterSuperintendentOrderStatusModel, InstituteList } from '../../../Models/CenterAllocationDataModels';
...
export class CenterSuperintendentAllocationComponent implements OnInit {
  ...
  async GetStaffTypeDDL(row: any) {
    try {
      ...
      await this.commonMasterService.CenterSuperitendentDDL(obj).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
  ...
  GetStatus(status: any) {
    try {
      let model = new CenterSuperintendentOrderStatusModel();
      model.PDFType = status;
      ...
      //
      this.centerAllocationService.GetStatusCenterSuperintendentOrder(model)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
