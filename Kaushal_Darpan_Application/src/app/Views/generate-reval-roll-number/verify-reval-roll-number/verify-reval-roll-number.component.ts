...
@Component({
  selector: 'app-verify-reval-roll-number',
  standalone: false,
  templateUrl: './verify-reval-roll-number.component.html',
  styleUrl: './verify-reval-roll-number.component.css'
})
export class VerifyRevalRollNumberComponent {
  ...
  async GetAllData() {
    try {
      ...
      //call
      await this.GetRollService.GetRevalRollNoData_Verify(this.searchRequest).then(
        (data: any) => {
          data = JSON.parse(JSON.stringify(data));
...
/*    console.log(this.sSOLoginDataModel.Mobileno, "MobileNo")*/
  ...
  GetPageName(tabno: number)
  {
    ...
    {
      this.PageNameTitile = 'Verified Reval Roll Number List'
    }
    else if (tabno == 3)
    {
      this.PageNameTitile = 'Reverted Reval Roll Number List'
    }
    else if (tabno == 4)
    {
      this.PageNameTitile = 'Published Reval Roll Number List'
    }
    else
    {
      this.PageNameTitile = 'Reval Roll Number List'
    }
  }
