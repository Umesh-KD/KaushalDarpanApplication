...
@Component({
    ...
    templateUrl: './add-branches-master.component.html',
    styleUrls: ['./add-branches-master.component.css'],
    standalone: false
})
export class AddBranchesMasterComponent implements OnInit {
  ...
  async GetByID(id: number) {
    try {
      ...
      await this.streamService.GetByID(id)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.request.StreamTypeID = data['Data']["StreamTypeID"];
          this.request.Name = data['Data']["Name"];
          this.request.Code = data['Data']["Code"];
          this.request.Duration = data['Data']["Duration"];
          this.request.StreamTypeID = data['Data']["StreamTypeID"];
  ...
  async saveData() {
    ...
    try {
      ...
      await this.streamService.SaveData(this.request)
        .then((data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
          if (this.State == EnumStatus.Success) {
            ...
            this.ResetControl();
            this.router.navigate(['/branchesmaster']);
          }
  ...
  async ResetControl() {
    ...
    this.request = new StreamMasterDataModels()
