...
export class StudentEnrollmentComponent {
  ...
  async SaveData_EditStudentDetails() {
    ...
    if (this.IsVerified && (this.sSOLoginDataModel.RoleID == EnumRole.Principal || this.sSOLoginDataModel.RoleID == EnumRole.PrincipalNon
      || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge_NonEng) && this.requestStudent.status == enumExamStudentStatus.EnrolledFeePaid) {
  ...
  async SaveDataMarked() {
    ...
    if (this.status != enumExamStudentStatus.RejectatBTER) {
      ...
      if (this.request.StudentFilterStatusId == enumExamStudentStatus.SelectedForEnrollment && this.status != enumExamStudentStatus.VerifiedForEnrollment && this.status != enumExamStudentStatus.EnrolledFeePaid) {

        this.toastr.error(" 'pay enrollment fee and verify each student' then choose 'Eligible For Enrollment' student!");
        return;
      }
    }
    ...
    try {
      ...
      if (this.status == enumExamStudentStatus.Dropout) {
        if (this.request.StudentFilterStatusId != enumExamStudentStatus.RejectatBTER && this.request.StudentFilterStatusId != enumExamStudentStatus.Dropout && (this.sSOLoginDataModel.RoleID == EnumRole.Principal || this.sSOLoginDataModel.RoleID == EnumRole.PrincipalNon) || (this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge_NonEng)) {
      ...
      if (this.request.StudentFilterStatusId == enumExamStudentStatus.Addimited || this.request.StudentFilterStatusId == enumExamStudentStatus.New_Addimited) {
        if (this.status == enumExamStudentStatus.SelectedForEnrollment && (this.sSOLoginDataModel.RoleID == EnumRole.Admin || this.sSOLoginDataModel.RoleID == EnumRole.AdminNon || this.sSOLoginDataModel.RoleID == EnumRole.ACP || this.sSOLoginDataModel.RoleID == EnumRole.ACP_NonEng)) {
          await this.SaveAdmittedStudentData();
        }
        else {
          this.toastr.error("Please choose 'Selected For Enrollment' Mark As with 'Admitted' status!");
        }
        return;
      }
    ...
    this.Swal2.Confirmation("Are you sure to continue?", async (result: any) => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          await this.studentEnrollmentService.SaveAdmittedStudentData(request)
            .then(async (data: any) => {
              this.State = data['State'];
              this.Message = data['Message'];
              this.ErrorMessage = data['ErrorMessage'];
    ...
    this.Swal2.Confirmation("Are you sure to continue?", async (result: any) => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          // Call service to save student exam status
          await this.studentEnrollmentService.SaveDropout(request)
            .then(async (data: any) => {
              this.State = data['State'];
              this.Message = data['Message'];
              this.ErrorMessage = data['ErrorMessage'];
  ...
  async GetDateConfig() {
    ...
    await this.commonMasterService.GetDateConfigSetting(data)
      .then((data: any) => {
        ...
        this.MapKeyEng = this.DateConfigSetting.ENROLLMENT;
        console.log(this.MapKeyEng, 'HEllo')
      }, (error: any) => console.error(error)
      );
  }
    ...
    this.Swal2.Confirmation("Are you sure to continue?", async (result: any) => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          // Call service to save student exam status
          await this.studentEnrollmentService.SaveRevokeDropout(request)
            .then(async (data: any) => {
              this.State = data['State'];
              this.Message = data['Message'];
              this.ErrorMessage = data['ErrorMessage'];
