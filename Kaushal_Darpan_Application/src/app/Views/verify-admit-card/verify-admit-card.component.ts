...
@Component({
  selector: 'app-verify-admit-card',
  standalone: false,
  templateUrl: './verify-admit-card.component.html',
  styleUrl: './verify-admit-card.component.css'
})
export class VerifyAdmitCardComponent {
...
/*    this.PDFType = Number(this.activeroute.snapshot.paramMap.get('id')??0)*/
  ...
  async GetAllData() {
    try {
      ...
      if (this.sSOLoginDataModel.RoleID == EnumRole.ACP || this.sSOLoginDataModel.RoleID == EnumRole.ACP_NonEng) {
        ...
        this.searchRequest.action = "_GetStatusVerifyAdmidCardListExaminer"
        this.searchRequest.Status = this.Status
        ...
        this.searchRequest.action = "_GetStatusVerifyAdmidCardListRegistrar"
        this.searchRequest.Status = this.Status
...
/*      this.SearchReqForEnroll.ModuleID = 2;*/
  ...
  ForwardRollNo() {
    ...
    this.Swal2.Confirmation("Are you sure you want to forward the Admit Card list for verification?", async (result: any) => {
      // Check if the user confirmed the action
      if (result.isConfirmed) {
        this.SaveApplicationWorkFlow(EnumEnrollNoStatus.Forwarded);
      }
    });
  ...
  async SaveApplicationWorkFlow(Action: number) {
    try {
      ...
      if (this.sSOLoginDataModel.RoleID == EnumRole.ACP || this.sSOLoginDataModel.RoleID == EnumRole.ACP_NonEng || Action == EnumEnrollNoStatus.Reverted || Action == EnumEnrollNoStatus.Verified
      ...
      if ((this.sSOLoginDataModel.RoleID == EnumRole.ACP && Action == EnumEnrollNoStatus.Forwarded) || (this.sSOLoginDataModel.RoleID == EnumRole.ACP_NonEng && Action == EnumEnrollNoStatus.Forwarded)) {
        this.IsExaminationVerified = false
        this.IsRegistrarVerified = false

      }
      if ((this.sSOLoginDataModel.RoleID == EnumRole.ACP && Action == EnumEnrollNoStatus.Published) || (this.sSOLoginDataModel.RoleID == EnumRole.ACP_NonEng && Action == EnumEnrollNoStatus.Published)) {
      ...
      if (this.sSOLoginDataModel.RoleID == EnumRole.Registrar || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge || this.sSOLoginDataModel.RoleID == EnumRole.Registrar_NonEng || this.sSOLoginDataModel.RoleID == EnumRole.ExaminationIncharge_NonEng) {
        Selected.forEach(e => {
  ...
  VerifyRollNumber() {
    ...
    if (this.ddlRollListStatus > 0) {
      if (this.ddlRollListStatus == EnumEnrollNoStatus.Reverted) {
        this.Swal2.Confirmation("Are you sure you want to revert Admit card list?", async (result: any) => {
          // Check if the user confirmed the action
          if (result.isConfirmed) {
      ...
      if (this.ddlRollListStatus == EnumEnrollNoStatus.Verified) {
        ...
        if (this.currentStatus == EnumEnrollNoStatus.Published) {
          ...
          this.Swal2.Confirmation("Are you sure you want to verified Admit card?", async (result: any) => {
            // Check if the user confirmed the action
            if (result.isConfirmed) {
