...
@Component({
  selector: 'app-published-enroll-no',
  standalone: false,
  templateUrl: './published-enroll-no.component.html',
  styleUrl: './published-enroll-no.component.css'
})
export class PublishedEnrollNoComponent {
  ...
  async GetAllData() {
    try {
      ...
      //call
      this.loaderService.requestStarted();
      await this.GetEnrollRollService.GetPublishedEnRollData(this.searchRequest)
        .then((data: any) => {
          debugger;
          data = JSON.parse(JSON.stringify(data));
  ...
  exportToExcel(): void {
    ...
    XLSX.writeFile(wb, 'PublishedEnrollmentNumberData.xlsx');
  }
  ...
  GetPageName(tabno: number) {
    if (tabno == 1) {
      ...
      this.PageNameTitile = 'Verified EnrollMent Number List'
    }
    else if (tabno == 3) {
