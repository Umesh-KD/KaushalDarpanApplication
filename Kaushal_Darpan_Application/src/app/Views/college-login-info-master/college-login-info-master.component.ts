...
import { ITITradeSearchModel, ITITradeDataModels } from '../../Models/ITITradeDataModels';
...
import { BTERMasterService } from '../../Services/BTER-Master/btermaster.service';

@Component({
  selector: 'app-college-login-info-master',
  standalone: false,
  templateUrl: './college-login-info-master.component.html',
  styleUrl: './college-login-info-master.component.css'
})

export class CollegeLoginInfoMasterComponent {
  ...
  constructor(
    ...
    private ITIMasterService: BTERMasterService,
  ) {
  }
  ...
  async GetCollegeLoginInfoByCode() {
    try {
      this.loaderService.requestStarted();
      await this.ITIMasterService.BTERGetCollegeLoginInfoByCode(this.requestCollege)
        .then((data: any) => {
  ...
  async onSearchCancel() {
    ...
    this.GetCollegeLoginInfoByCode();
  }
  ...
  async GeITI_CollegeLoginInfo() {
    ...
    try {
      ...
      this.loaderService.requestStarted();
      await this.ITIMasterService.GetBTER_CollegeLoginInfoMaster(this.requestCollege).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
        this.ITI_CollegeLoginInfoList = data.Data;
        this.loadInTable();
      ...
      this.Swal2.Confirmation("Are you sure you want to  Rest SSOID & Password ?",
        ...
        async (result: any) => {
          ...
          if (result.isConfirmed) {
            ...
            this.requestCollege.DepartmentID = 1;
            this.loaderService.requestStarted();

            await this.ITIMasterService.BTERUpdate_CollegeLoginInfo(this.requestCollege)
              .then((data: any) => {
                this.State = data['State'];
                this.Message = data['Message'];
                this.ErrorMessage = data['ErrorMessage'];
  ...
  exportToExcel(): void {
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'CollegeLoginInfoList.xlsx');
  }
