...
@Component({
  selector: 'app-reval-add-examiner',
  templateUrl: './reval-add-examiner.component.html',
  styleUrls: ['./reval-add-examiner.component.css'],
  standalone: false
})
export class RevalAddExaminerComponent implements OnInit {
  ...
  async getGroupCodeMasterList() {
    try {
      ...
      //call
      await this.commonMasterService.GetExaminerGroupCode_Reval(model).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
  ...
  async getStaffForExaminerData() {
    try {
      ...
      this.searchRequest.IsReval = true;
      //
  ...
  async appointStaffAsExaminer(StaffID: number) {
    ...
    this.AppointExaminer.RoleID = this.sSOLoginDataModel.RoleID;

    const saveExaminer = async () => {
      try {
        this.loaderService.requestStarted();
        const data: any = await this.examinerservice.SaveExaminerData_Reval(this.AppointExaminer);
  ...
  async GetByID(id: number, StaffSubjectId:number) {
    try {
      ...
      await this.examinerservice.Getexaminer_byID_Reval(id, StaffSubjectId, this.sSOLoginDataModel.DepartmentID, this.sSOLoginDataModel.EndTermID, this.sSOLoginDataModel.Eng_NonEng)
        .then(async (data: any) => {
