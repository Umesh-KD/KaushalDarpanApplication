...
import { SerialMasterConfigurationBterModel } from '../../../Models/MasterConfigurationBterModel';
import { MasterConfigurationBterService } from '../../../Services/MasterConfigurationBter/master-configuration-bter.service';

@Component({
  selector: 'app-serial-master-bter',
  templateUrl: './serial-master-bter.component.html',
  styleUrls: ['./serial-master-bter.component.css'],
  standalone: false
})
export class SerialMasterBTERComponent implements OnInit {
  public SerialMasterFromGroup!: FormGroup;

  public isFormSubmitted: boolean = false;
  public isLoading: boolean = false;
  public request = new SerialMasterConfigurationBterModel();
  ...
  constructor(
    ...
    private mstConfigService: MasterConfigurationBterService,
 ...
 /*   }*/
  ...
  async openModalGenerateOTP(content: any, item: SerialMasterConfigurationBterModel)
  {
    this.isFormSubmitted = true;
    this.RemoveValidation();
    if (this.SerialMasterFromGroup.invalid) {
  ...
  async GetConfigType() {
    ...
    try {
      ...
      await this.commonMasterService.GetConfigurationType(this.RoleID, EnumConfigTypeTabs.Serial_Tab).then((data: any) => {
        ...
        console.log(this.TypeMasterDDL,"TypeList")
      }, (error: any) => console.error(error))
    }
    catch (ex) {
      console.log(ex);
  ...
  ShowValidColumn() {
    return this.TypeMasterDDL.filter(function (x) { return (x.ID == EnumConfigurationType.ApplicationNo || x.ID == EnumConfigurationType.ChallanNo || x.ID == EnumConfigurationType.ApplicationNo_DirectAdmission || x.ID == EnumConfigurationType.ChallanNo_DirectAdmission) }).length > 0;
  }
  ...
  async ResetControls() {
    ...
    this.request = new SerialMasterConfigurationBterModel()
    this.request.ModifyBy = this.sSOLoginDataModel.UserID
    this.request.DepartmentID = this.sSOLoginDataModel.DepartmentID
    this.request.CourseTypeID = this.sSOLoginDataModel.Eng_NonEng
    this.request.EndTermID = this.sSOLoginDataModel.EndTermID;
  ...
  RemoveValidation() {
    //validatiion
    if (this.request.TypeID == EnumConfigurationType.ApplicationNo || this.request.TypeID == EnumConfigurationType.ChallanNo || this.request.TypeID == EnumConfigurationType.ApplicationNo_DirectAdmission || this.request.TypeID == EnumConfigurationType.ChallanNo_DirectAdmission || this.request.TypeID == EnumConfigurationType.CCCode) {
    ...
    if (this.request.TypeID != EnumConfigurationType.CCCode) {
      ...
      this.SerialMasterFromGroup.controls['startFrom'].clearValidators();

    } else {
      this.SerialMasterFromGroup.controls['txtlength'].setValidators([DropdownValidators])
      this.SerialMasterFromGroup.controls['startFrom'].setValidators([Validators.required])
    }
    this.SerialMasterFromGroup.controls['txtlength'].updateValueAndValidity();
    this.SerialMasterFromGroup.controls['startFrom'].updateValueAndValidity();
  }
