...
@Component({
  selector: 'app-reval-appoint-examiner-list',
  templateUrl: './reval-appoint-examiner-list.component.html',
  styleUrls: ['./reval-appoint-examiner-list.component.css'],
    standalone: false
})
export class RevalAppointExaminerListComponent {
  ...
  async getStaffForExaminerData() {
    try {
      ...
      //
      await this.examinerservice.GetRevalTeacherForExaminer(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
