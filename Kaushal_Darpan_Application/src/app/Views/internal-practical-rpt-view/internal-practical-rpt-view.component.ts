...
@Component({
  selector: 'app-internal-practical-rpt-view',
  standalone: false,
  templateUrl: './internal-practical-rpt-view.component.html',
  styleUrl: './internal-practical-rpt-view.component.css'
})

export class InternalPracticalRptViewComponent implements OnInit {
  ...
  async OnSubmit(StudentExamPaperMarksID: number = 0) {
    ...
    for (let x of filtered) {
      if (this.searchRequest.InternalPracticalID === 2) {
        ...
        if (x.IsPresentInternalAssisment === 0 && (x.MaxInternalAssisment !== 0 || x.ObtainedInternalAssisment !== 0)) {
      ...
      if (this.searchRequest.InternalPracticalID === 1) {
        ...
        // Other validations (absent, present, etc.)
        if (x.IsPresentPractical === 0 && (x.MaxPractical !== 0 || x.ObtainedPractical !== 0)) {
          this.toastr.error('For absent students, marks must be 0.');
          return;
        }
  ...
  async onStatusPracticalAssesmentChange(dOC: any) {
  ...
  async onStatusPracticalChange(dOC: any) {
