...
@Component({
  selector: 'app-user-master-office-wise',
  standalone: false,
  templateUrl: './user-master-office-wise.component.html',
  styleUrl: './user-master-office-wise.component.css'
})
export class UserMasterOfficeWiseComponent {
  ...
  async GetMasterData() {
    try {
      ...
      await this.commonMasterService.InstituteMaster(this.sSOLoginDataModel.DepartmentID, 0, this.sSOLoginDataModel.EndTermID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  toggleCheckbox(role: any): void {
    ...
    if(role.Marked == false) {
      role.IsMainRole = false;
    }
  }
  ...
  async GetUserMasterList() {
    try {
      ...
      this.searchRequest.ParentRoleID = this.sSOLoginDataModel.RoleID;
      await this.UserMasterService.GetUserData_RoleWise(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async SaveData_AssignRole() {
    try {
      ...
      if (hasInvalidRole) {
        this.toastr.error("Please Select Institute for Principle Role");
        return;
      }
  ...
  async GetByID(UserID: number) {
    try {
      await this.UserMasterService.GetByID(UserID).then(async (data: any) => {
        data = JSON.parse(JSON.stringify(data));
