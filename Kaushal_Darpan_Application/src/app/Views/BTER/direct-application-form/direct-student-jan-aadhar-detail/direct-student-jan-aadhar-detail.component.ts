...
@Component({
  selector: 'app-direct-student-jan-aadhar-detail',
  templateUrl: './direct-student-jan-aadhar-detail.component.html',
  styleUrls: ['./direct-student-jan-aadhar-detail.component.css'],
  standalone: false
})
export class DirectDTEStudentJanAadharDetailComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.IsDirectAdmission) {
      ...
      this.StudentJanDetailFormGroup.valueChanges.pipe(
    ...
    if (this.sSOLoginDataModel.ApplicationID > 0) {
      ...
      this.model.ApplicationID = this.sSOLoginDataModel.ApplicationID;
      await this.GetById();
      ...
      if (originalValue) {
        ...
        setTimeout(() => {
          ...
          if (formControlName) {
            ...
            this.StudentJanDetailFormGroup.get(formControlName)?.setValue(translatedText, { emitEvent: false });
          }
  ...
  async SaveData() {
    try {
      ...
      //if (this.model.CategoryA != 5 ||  this.model.IsDevnarayan == 0) {
      //  this.model.IsDevnarayan = 0
      //  this.model.DevnarayanTehsilID = 0
      //  this.model.DevnarayanDistrictID = 0
      ...
      //if (this.model.CategoryA == 5 && this.model.IsDevnarayan == 1) {
      ...
      if (this.model.CourseType != 1) {
        this.StudentJanDetailFormGroup.get('txtDOB')?.setValidators(this.minimumAgeValidator(10));
        this.StudentJanDetailFormGroup.get('txtDOB')?.updateValueAndValidity();
      }
      ...
      /* ---***Condition is comment as request recived from cleind that no need to set application to GENERAL as form checker will do that****--
      if (this.model.CategoryA == 4 || this.model.CategoryA == 9 || this.model.CategoryA == 5) {
        const admissionDate = new Date(this.AdmissionDateList[0].From_Date); // Convert From_Date to Date object
        const certificateGenerationDate = new Date(this.model.CertificateGeneratDate); // Convert CertificateGeneratDate to Date object
        var differenceInYears: number = 0;
        differenceInYears = (admissionDate.getTime() - certificateGenerationDate.getTime()) / (1000 * 3600 * 24 * 365.25);
        if (differenceInYears > 3) {
          await this.Swal2.Confirmation("If the certificate issue date is more than 3 years ago, you will be considered in the general category. Are you sure you want to Submit this ?",
            async (result: any) => {
              if (result.isConfirmed) {
                this.model.CategoryA = 1;
                this.submitFormdata();
  ...
  async submitFormdata() {
    ...
    this.model.InstituteID = this.sSOLoginDataModel.InstituteID;
    await this.StudentJanAadharDetailService.SaveDTEDirectApplicationData(this.model)
      .then((data: any) => {
        this.State = data['State'];
        ...
        if (this.State == EnumStatus.Success) {
          ...
          this.formSubmitSuccess.emit(true); // Notify parent of success
          this.tabChange.emit(1); // Move to the next tab (index 1)
