...
@Component({
  selector: 'app-direct-optional-form',
  templateUrl: './direct-optional-form.component.html',
  styleUrls: ['./direct-optional-form.component.css'],
  standalone: false
})
export class DirectOptionalFormComponent implements OnInit {
  ...
  async GetCollegeDetails() {
    ...
    try {
      this.loaderService.requestStarted();
      await this.commonFunctionService.GetCollegeDetails(this.SSOLoginDataModel.InstituteID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data?.Data?.length > 0) {
            this.GetDdlBindList = data['Data'];
            this.formData.DistrictID = data['Data'][0]['DistrictID'];
            this.formData.College_TypeID = data['Data'][0]['InstitutionManagementTypeID'];
  ...
  async GetCollegeBranchDetails() {
    ...
    try {
      ...
      this.collegeSearchRequest.ApplicationID = this.SSOLoginDataModel.ApplicationID;
      this.collegeSearchRequest.action = "_getDatabyDIRECTCollege";
      await this.commonFunctionService.BterCollegesGetAllData(this.collegeSearchRequest)
  ...
  AddChoice() {
    ...
    if (this.AddedChoices.length >= 1) {
      this.toastr.warning("आप केवल एक ही विकल्प जोड़ सकते हैं");
      return;
    }
