...
@Component({
  selector: 'app-dtereturn-items-list',
  templateUrl: './dtereturn-items-list.component.html',
  styleUrls: ['./dtereturn-items-list.component.css'],
  standalone: false
})
export class DteReturnItemsListComponent {
  ...
  async GetAllData() {
    ...
    try {
      ...
      await this.dteIssuedItemsService.GetAllRetunItem(this.Searchrequest)
        .then((data: any) => {
  ...
  async SaveData_ReturnItem() {
    ...
    this.loaderService.requestStarted();
    try {
      await this.dteIssuedItemsService.SaveDataReturnDTEItem(this.Returnrequest)
        .then(async (data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
