...
@Component({
  selector: 'app-BTERUpwardList',
  standalone: false,
  templateUrl: './BTERUpwardList.component.html',
  styleUrl: './BTERUpwardList.component.css'
})

export class BTERUpwardListComponent implements OnInit {
  ...
  async exportToExcel() {
    ...
    try {
      ...
      this.searchRequest.action = "UPWARD_LIST";
      ...
      await this.AllotmentService.ShowAllotmentDataList(this.searchRequest)
        .then((data: any) => {
          ...
          if (data.State === EnumStatus.Success) {
            ...
            const wb: XLSX.WorkBook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
            XLSX.writeFile(wb, 'UpwardStudentList.xlsx');
