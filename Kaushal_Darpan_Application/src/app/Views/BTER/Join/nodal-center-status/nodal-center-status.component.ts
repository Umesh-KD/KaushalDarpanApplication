...
@Component({
  selector: 'app-nodal-center-status',
  standalone: false,

  templateUrl: './nodal-center-status.component.html',
  styleUrl: './nodal-center-status.component.css'
})
export class NodalCenterStatusComponent {
  ...
  public ReportingStatus: any = [{ Name: "Accept", Id: 4 }, { Name: "Reject", Id: 7 }, { Name: "Move to 2nd Round", Id: 11 }, { Name: "Not Reported", Id: 5 }]
  ...
  async GetDateConfig() {
    var data = {
      ...
      Key: "FIRST ALLOTMENT REPORTING,FIRST UPWARD REPORTING,SECIND ALLOTMENT REPORTING,FIRST UPWARD REPORTING,INTERNAL SLIDING REPORTING",
                ...
                this.commonMasterService.GetBTEROriginalDocument(listModel)
                  .then((data: any) => {
                    ...
                    if (data.State == EnumStatus.Success) {
                      this.OriginalDocumentList = data.Data
                    } else {
  ...
  DownloadFile(FileName: string, DownloadfileName: any): void {
    debugger
    const fileUrl = this.appsettingConfig.StaticFileRootPathURL + "/" + this.request.FolderName + "/" + FileName;
    // const fileUrl = this.appsettingConfig.StaticFileRootPathURL + "/" + GlobalConstants.ReportsFolder + "/" + FileName;
  ...
  async SaveData() {
    ...
    try {
      await this.StudentsJoiningStatusMarksService.NodalReporting(this.request)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
