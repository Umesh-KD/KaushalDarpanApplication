...
/*    this.searchRequest.CourseTypeId = Number(this.route.snapshot.paramMap.get('id') ?? 0);*/
  ...
  AllotmentChange() {
    if (this.searchRequest.AllotmentMasterId == 2) {
      this.AllotmentKey = "INSTITUTE REPORTING";
  ...
  async GetDateConfig() {
    var data = {
      ...
      Key: "INSTITUTE REPORTING,INSTITUTE REPORTING,INSTITUTE REPORTING",
      ...
      if ((this.OTP == GlobalConstants.DefaultOTP) || (this.OTP == this.GeneratedOTP)) {
        try {
          ...
          if (this.searchRequest.CourseTypeId == 1) {
            this.router.navigate(['/BterallotmentreportingENG/1'], {
              queryParams: { AllotmentID: this.request.AllotmentId }
            });
          } else if (this.searchRequest.CourseTypeId == 2) {
            this.router.navigate(['/BterallotmentreportingNonENG/2'], {
              queryParams: { AllotmentID: this.request.AllotmentId }
            });
          } else if (this.searchRequest.CourseTypeId == 3) {
            this.router.navigate(['/BterallotmentreportingLateral/3'], {
              queryParams: { AllotmentID: this.request.AllotmentId }
            });
          }
    ...
    try {
      ...
      await this.reportService.DownloadAllotmentLetter(id)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
            this.DownloadFile(data.Data, 'Allotment-Report');
          }
          else {
  ...
  async DownloadRepoprtingReceipt(id: any) {
    try {
      this.loaderService.requestStarted();
      //this.ApplicationID = this.StudentsJoiningStatusMarksList[0].AllotmentId;
      await this.reportService.DownloadReportingLetter(id)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
            this.DownloadFile(data.Data, 'Repoprting-Receipt');
          }
          else {
            this.toastr.error(data.ErrorMessage)
          }
  ...
  redirect(AllotmentId: number) {
    if (this.searchRequest.CourseTypeId == 1) {
      ...
      this.router.navigate(['/BterallotmentreportingDegreeNonENG/4'], {
        queryParams: { AllotmentID: AllotmentId }
      });
    } else if (this.searchRequest.CourseTypeId == 5) {
      this.router.navigate(['/BterallotmentreportingDegreeLatralNonENG/5'], {
        queryParams: { AllotmentID: AllotmentId }
      });
    }
