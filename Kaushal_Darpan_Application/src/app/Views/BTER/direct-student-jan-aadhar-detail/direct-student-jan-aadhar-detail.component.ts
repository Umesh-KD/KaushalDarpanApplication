...
export class DirectStudentJanAadharDetailComponent implements OnInit {
  ...
  CourseTypeChange() {
    this.GetCollegeBranchDetails();
  }


  async GetCollegeBranchDetails() {
    
    try {
      this.loaderService.requestStarted();
      this.collegeSearchRequest.InstituteID = this.sSOLoginDataModel.InstituteID;
      this.collegeSearchRequest.StreamType = this.sSOLoginDataModel.Eng_NonEng;
      this.collegeSearchRequest.ApplicationID = 1;
      this.collegeSearchRequest.action = "_getDatabyDIRECTCollege";

      await this.commonMasterService.BterCollegesGetAllData(this.collegeSearchRequest)
  ...
  async GetMasterData() {
    try {
      ...
      await this.commonMasterService.GetCommonMasterData('GetDirectAdmissionCourseType', this.sSOLoginDataModel.InstituteID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  redirectApplication() {
    ...
    if (this.model.coursetype == undefined || this.model.coursetype == 0) {
      this.toastr.error("Please select course")
      return;
    }
    else if (this.model.MobileNumber == undefined || this.model.MobileNumber == "") {
      this.toastr.error("Please enter mobile number.")
      return;
    }
    debugger;

    this.sSOLoginDataModel.Eng_NonEng = this.model.coursetype;
    //this.sSOLoginDataModel.Eng_NonEngName = this.courseTypeList.find((x: { value: number; }) => x.value === this.model.coursetype)?.key;
    this.sSOLoginDataModel.Eng_NonEngName = this.courseTypeList.find((x: { ID: number }) => x.ID == this.model.coursetype)?.Name;
    ...
    this.sSOLoginDataModel.ApplicationFinalSubmit = 0
    if (this.model.MobileNumber !== undefined) {
      this.sSOLoginDataModel.PrincipleMobile = this.sSOLoginDataModel.Mobileno;
      this.sSOLoginDataModel.Mobileno = this.model.MobileNumber;
      
    }
    //this.sSOLoginDataModel.Mobileno = this.model.MobileNumber ?? '';
    this.sSOLoginDataModel.DepartmentID = 1;
    
    this.sSOLoginDataModel.PrincipleSSOID = this.sSOLoginDataModel.SSOID;
    //this.sSOLoginDataModel.ApplicationID = item.ApplicationID;
    this.sSOLoginDataModel.SSOID = this.model.MobileNumber;
    ...
    this.routers.navigate(['/DirectDTEApplicationform']);
  }
