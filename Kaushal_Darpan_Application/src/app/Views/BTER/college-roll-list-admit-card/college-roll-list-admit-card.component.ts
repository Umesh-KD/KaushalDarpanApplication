...
@Component({
  selector: 'app-college-roll-list-admit-card',
  standalone: false,
  templateUrl: './college-roll-list-admit-card.component.html',
  styleUrl: './college-roll-list-admit-card.component.css'
})
export class CollegeRollListAdmitCardComponent {
  ...
  public rollNoDownloadReq = new DownloadnRollNoModel();
  public admitCardDownloadReq = new GenerateAdmitCardSearchModel();
  ...
  async GetCollegeMasterList(i: any) {
    ...
    try {
      this.loaderService.requestStarted();
      await this.getRollService.GetCollegeRollListAndAdmitCard(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async DownloadStudentRollNumber() {
    try {
      this.loaderService.requestStarted();
      this.rollNoDownloadReq.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      this.rollNoDownloadReq.EndTermID = this.sSOLoginDataModel.EndTermID;
      this.rollNoDownloadReq.Eng_NonEng = this.sSOLoginDataModel.Eng_NonEng;
      this.rollNoDownloadReq.RoleID = this.sSOLoginDataModel.RoleID;

      await this.reportService.DownloadStudentRollNumber_InsituteWise(this.rollNoDownloadReq).then(async (data: any) => {
        data = JSON.parse(JSON.stringify(data));
  ...
  async GetStudentAdmitCardBulk() {
    try {
      this.loaderService.requestStarted();
      this.admitCardDownloadReq.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      this.admitCardDownloadReq.EndTermID = this.sSOLoginDataModel.EndTermID;
      this.admitCardDownloadReq.Eng_NonEng = this.sSOLoginDataModel.Eng_NonEng;
      this.admitCardDownloadReq.RoleID = this.sSOLoginDataModel.RoleID;

      await this.reportService.GetStudentAdmitCardBulk_InstituteWise(this.admitCardDownloadReq).then(async (data: any) => {
        data = JSON.parse(JSON.stringify(data));
        console.log(data, "Data");
        if (data.State == EnumStatus.Success) {
          //this.DownloadFile(data.Data);
