...
//import { EnumDeploymentStatus, EnumRole, EnumStatus } from '../../Common/GlobalConstants';
import { ReportService } from '../../../../../Services/Report/report.service';
import { EnumEMProfileStatus, EnumProfileStatus, EnumRole, EnumStatus, GlobalConstants } from '../../../../../Common/GlobalConstants';

@Component({
  selector: 'app-request-list',
  standalone: false,

  templateUrl: './Bter-Em-DepartmentWiseRequest-list.component.html',
  styleUrl: './Bter-Em-DepartmentWiseRequest-list.component.css'
})
export class BterEmDepartmentWiseRequestlistComponent implements OnInit {
  ...
  groupFormVRSDteApprove!: FormGroup;
  groupFormVRSAccountant !: FormGroup;
  ...
  @ViewChild('MyModel_VRS') MyModel_VRS: any;
  @ViewChild('MyModel_VRSDTEApprove') MyModel_VRSDTEApprove: any;
  @ViewChild('MyModel_VRSAccountant') MyModel_VRSAccountant: any;
  ...
  async ngOnInit() {
    this.groupForm = this.fb.group({
      ...
      txtLastworkingDate: [this.today],
      txtJoiningDate: [''],
    });


    this.groupFormVRSDteApprove = this.fb.group({
      ddlStatus: [0, [DropdownValidators]],
      txtRemark: ['', Validators.required],
      txtIsAccount: [false],
      txtAccountComments: ['']
    ...
    this.groupFormVRSAccountant = this.fb.group({
      txtIsAccount: [false],
      txtAccountComments: ['']
    });
    ...
    try {
      ...
      await this.commonMasterService.AllDDlManageByTypeCommanMaster(this.type)
        .then((data: any) => {
          ...
          this.filteredStatusList = this.filteredStatusList.filter((item: any) => item.ID != EnumEMProfileStatus.Pending && item.ID != EnumEMProfileStatus.Completed && item.ID !=EnumEMProfileStatus.LockAndSubmit && item.ID != EnumEMProfileStatus.Revert)
  ...
  async onSubmitModel_VRSApproveDTE(model: any, userSubmitData: any) {
    debugger
    try {
      this.RowlistData = { ...userSubmitData };
      console.log(this.RequestUpdateStatus, "modal");
  ...
  async onSubmitModel_VRSAccountant(model: any, userSubmitData: any) {
    debugger
    try {
      this.RowlistData = { ...userSubmitData };
      console.log(this.RequestUpdateStatus, "modal");
  ...
  async updateReqStatus() {
    ...
    try {
      ...
      if (this.searchRequest.RequestType == 1) {
        await this.userRequestService.BterEmUserRequestUpdateStatus(this.RequestUpdateStatus)
          .then(async (data: any) => {
  ...
  async DownloadRelievingLetter(UserID: number) {
    try {
      this.searchRequestRelieving.UserID = UserID;
      this.loaderService.requestStarted();
      await this.reportService.BterRelievingLetter(this.searchRequestRelieving)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          console.log("DownloadRelievingLetter", data)
  ...
  async updateReqStatusVRSApproveDTE() {
    debugger


    this.isSubmitted = true;
    if (this.groupFormVRSDteApprove.invalid) {
      return console.log("error")
    }
    this.loaderService.requestStarted();
    this.isLoading = true;
    ...
    try {
      ...
      this.RequestUpdateStatus.RequestType = this.RowlistData.RequestTypeID;
      this.RequestUpdateStatus.UserID = this.RowlistData.UserID;
      this.RequestUpdateStatus.RoleID = this.sSOLoginDataModel.RoleID;
  ...
  async updateReqStatusVRSAccountant() {
    debugger


    this.isSubmitted = true;
    if (this.groupFormVRSAccountant.invalid) {
      return console.log("error")
    }
    this.loaderService.requestStarted();
    this.isLoading = true;
  ...
  async IsEOLValidation() {
    if (this.RequestUpdateStatus.IsEOL == true) {
      this.groupFormVRS.controls['txtEOLFromDate'].setValidators(Validators.required);
      this.groupFormVRS.controls['txtEOLToDate'].setValidators(Validators.required);
    }else{
      this.groupFormVRS.controls['txtEOLFromDate'].clearValidators();
      this.groupFormVRS.controls['txtEOLToDate'].clearValidators();
    }
    this.groupFormVRS.controls['txtEOLFromDate'].updateValueAndValidity();
    this.groupFormVRS.controls['txtEOLToDate'].updateValueAndValidity();
  }


  async IsEnquiriesValidation() {
    if (this.RequestUpdateStatus.IsEnquiries == true) {
      this.groupFormVRS.controls['txtRemark'].setValidators(Validators.required);
     
    } else {
      this.groupFormVRS.controls['txtRemark'].clearValidators();
    
    }
    this.groupFormVRS.controls['txtRemark'].updateValueAndValidity();
  
  }

  async IsDteAdminValidation() {
    if (this.RequestUpdateStatus.IsEnquiries == true) {
      this.groupFormVRSDteApprove.controls['txtAccountComments'].setValidators(Validators.required);

    } else {
      this.groupFormVRSDteApprove.controls['txtAccountComments'].clearValidators();

    }
    this.groupFormVRSDteApprove.controls['txtAccountComments'].updateValueAndValidity();

  }


  async IsAccountValidation() {
    if (this.RequestUpdateStatus.IsEnquiries == true) {
      this.groupFormVRSAccountant.controls['txtAccountComments'].setValidators(Validators.required);

    } else {
      this.groupFormVRSAccountant.controls['txtAccountComments'].clearValidators();

    }
    this.groupFormVRSAccountant.controls['txtAccountComments'].updateValueAndValidity();
