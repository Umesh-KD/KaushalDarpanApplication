...
@Component({
  selector: 'app-bter-request-list',
  standalone: false,
  
  templateUrl: './bter-request-list.component.html',
  styleUrl: './bter-request-list.component.css'
})
export class BterUserRequestListComponent implements OnInit {
  ...
  async DeleteRow(item: ITIGovtEMStaff_EducationalQualificationAndTechnicalQualificationModel) {
    ...
    //          this.loaderService.requestStarted();
    //          /*     alert(isParent)*/
    //          await this.ITIGovtEMStaffMasterService.ITI_Govt_EM_EducationalQualificationDeleteByID()
    //            .then(async (data: any) => {
    //              data = JSON.parse(JSON.stringify(data));
    //              console.log(data)
  ...
  nextData() {
    if (this.totalShowData < Number(this.UserRequestList[0]?.TotalRecords)) {
      if (this.pageNo >= 1) {
  ...
  async RelievingLetter(UserID: number) {
    ...
    try {
      ...
      console.log('main data ===>', this.searchRequestRelieving);
    } catch (Ex) {
      console.log(Ex);
    } finally {
      setTimeout(() => {
  ...
  async updateReqStatus() {
    ...
    try {
      ...
      this.RequestUpdateStatus.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      this.RequestUpdateStatus.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      await this.userRequestService.BterEmStafffJoiningRequestUpdateAndPromotions(this.RequestUpdateStatus)
        .then(async (data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
  ...
  async EditInfo(id: number) {
    try {
      ...
      this.routers.navigate(['/Bter-AddUserRequest', id]);
