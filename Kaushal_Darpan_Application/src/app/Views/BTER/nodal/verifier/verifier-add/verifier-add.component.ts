...
import { NodalVerifierDataModel, VerifierApiDataModel, VerifierDataModel } from '../../../../../Models/VerifierDataModel';
...
@Component({
  selector: 'app-verifier-add',
  standalone: false,
  templateUrl: './verifier-add.component.html',
  styleUrl: './verifier-add.component.css'
})
export class VerifierAddComponent implements OnInit {
  public SSOLoginDataModel = new SSOLoginDataModel();
  public VerifierFormGroup!: FormGroup;
  public request = new NodalVerifierDataModel();
  ...
  async SaveVerifierData() {
    ...
    try {
      ...
      if (this.request.DepartmentID == EnumDepartment.BTER) {
        ...
        this.request.RoleID = EnumRole.DTENodalVerifier
      } else {
        this.request.UserID = this.SSOLoginDataModel.UserID
        this.request.RoleID = EnumRole.DTENodalVerifier
      }
      ...
      this.request.CourseType = this.SSOLoginDataModel.Eng_NonEng
      if (this.request.ActiveStatus == false) {
        this.request.DeleteStatus = true;
      }

      await this.verifierService.SaveDTENodalVerifierData(this.request).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
        if (data.State == EnumStatus.Success) {
          this.toastr.success(data.Message)
          this.ResetControl()
          this.router.navigate(['/nodal-verifier']);
        } else {
  ...
  async ResetControl() {
    ...
    this.request = new NodalVerifierDataModel();
