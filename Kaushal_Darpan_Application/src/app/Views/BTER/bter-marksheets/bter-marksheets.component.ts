...
@Component({
  selector: 'app-bter-marksheets',
  standalone: false,
  templateUrl: './bter-marksheets.component.html',
  styleUrl: './bter-marksheets.component.css'
})
export class BterMarksheetsComponent implements OnInit {
  ...
  loadMasterData(): void {
    ...
    this.ReportTypelist = [
      { ID: 1, Name: 'Download Report 5th Sem Back Paper', URL: '5th-sem-back-paper-report' }
    ];
  }
  ...
  async GetAllData(): Promise<void> {
    ...
    if(requestData.Type == "5th-sem-back-paper-report"){
      requestData.EndTermID = this.ssoLoginUser.EndTermID
    }
  ...
  onChangeType() {
    ...
    if(this.filterForm.value.Type == "5th-sem-back-paper-report"){
      this.filterForm.get('EndTerm')?.disable();
      this.filterForm.get('selectedInstitute')?.disable();
      this.filterForm.get('selectedSemester')?.disable();
    }
  }
}
