...
@Component({
    selector: 'app-campus-post',
    templateUrl: './campus-post.component.html',
    styleUrls: ['./campus-post.component.css'],
    standalone: false
})
export class CampusPostComponent implements OnInit {
  ...
  Companyrequest = new CompanyMasterDataModels();
  request_EligibilityCriteriaModel = new CampusPostMaster_EligibilityCriteriaModel();
  ...
  async ngOnInit() {
    ...
    /*  txtNoofPositions: ['', Validators.required],*/
      /*txtNoofPositions: ['', Validators.required],*/
      txtNoofPositions: ['', [DropdownValidators]],
    ...
    if (this.request_EligibilityCriteriaModel.PassingYear != null && this.request_EligibilityCriteriaModel.PassingYear != 0) {
      this.onPassingYearChange();
    }

    if (this.request_EligibilityCriteriaModel.ToPassingYear != null && this.request_EligibilityCriteriaModel.ToPassingYear != 0) {
      this.onPassingYearChange();
    }

    if (this.request_EligibilityCriteriaModel.AgeAllowedFrom != null && this.request_EligibilityCriteriaModel.AgeAllowedFrom != '')
    {
        ...
        .then(async (data: any) =>
        {
         ...
         console.log(data)
          ...
          if (data['Data']?.length > 0) {
            ...
            this.request.CompanyTypeID = data['Data'][0]["CompanyTypeID"];
  ...
  toggleIsMainRole(row: CampusPostMasterModel): void
  {
    ;
  ...
  async AddNewRole() {
    ...
    //  this.toastr.error("Please Select Correct Age from and to")
    //  return
    //}
    //if (this.MinAge < 18) {
    ...
    //  this.toastr.warning("Max age cannot be less than Min Age")
    //  return
    //}

    
    //if (this.request_EligibilityCriteriaModel.AgeAllowedFrom > this.request_EligibilityCriteriaModel.AgeAllowedTo) {
    //  this.toastr.warning("Age allowed From show be ")
    //}
    console.log("this.Branch", this.BranchList)
    ...
    try {
      ...
      this.BranchList.forEach((branch: any) => {
        ...
        const isDuplicateBranch = this.request.EligibilityCriteriaModel.some(
          (item: any) => item.BranchID === branch.StreamID
        );

        if (isDuplicateBranch) {
          this.toastr.error(`You Can Not Add Branch: ${branch.StreamName} again.`);
          return; // Skip this branch, continue with others if needed
        }
        ...
        this.request.EligibilityCriteriaModel.push(
          {
            ...
            BranchID: branch.StreamID,
            BranchName: branch.StreamName,
  ...
  async btnRowDelete_OnClick(item: CampusPostMaster_EligibilityCriteriaModel) {
    try {
      this.loaderService.requestStarted();
