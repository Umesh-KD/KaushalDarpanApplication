...
@Component({
  selector: 'app-admin-sca-marking',
  standalone: false,
  templateUrl: './admin-sca-marking.component.html',
  styleUrl: './admin-sca-marking.component.css'
})
export class AdminSCAMarkingComponent {
  ...
  async GetGradeList() {
    ...
    try {
      ...
      //
      this.loaderService.requestStarted();
      await this.SCAService.GetAllDataSCA_Admin(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async OnSubmit(StudentExamPaperMarksID: number = 0, isFinalSubmit: boolean = false) {
    try {
      ...
      // **Save data**
      await this.SCAService.UpdateSaveDataSCA_Admin(finalSubmissionList)
        .then(async (data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
