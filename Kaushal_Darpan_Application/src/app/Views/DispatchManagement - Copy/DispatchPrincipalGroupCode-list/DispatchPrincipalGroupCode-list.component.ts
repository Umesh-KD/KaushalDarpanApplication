...
@Component({
  selector: 'app-DispatchPrincipalGroupCode-list',
  standalone: false,
  templateUrl: './DispatchPrincipalGroupCode-list.component.html',
  styleUrl: './DispatchPrincipalGroupCode-list.component.css'
})
export class DispatchPrincipalGroupCodeListComponent {
  ...
  async getgroupData() {
    ...
    if (this.searchRequest.Status == this._DispatchDDlValue.Pending) {
      this.IsPending = true;
    } else {
      this.IsPending = false;
    }
    ...
    try {
      await this.DispatchService.GetDispatchGroupcodeList(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
        this.DispatchPrincipalGroupCodeList1 = data.Data;
        console.log(this.StatusID, "status")
        let sID = 0;  // Use let instead of const if the value is changing
        if (this.searchRequest.Status == 1) {
          sID = 0;
          this.IsPending = true;

        } else {
          sID = this.searchRequest.Status;
        }
  ...
  async CheckOtpAfterSave() {
    ...
    for (const item of selectedItems) {
      this.UpdateStatusDispatchlist.push({
        ...
        Status: this._DispatchDDlValue.SendtoBterFromCollagePrincipal,
        Action: '',
    ...
    this.isLoading = true;
    try {
      await this.DispatchService.OnSTatusUpdateDispatchl(this.UpdateStatusDispatchlist)
        .then(async (data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
