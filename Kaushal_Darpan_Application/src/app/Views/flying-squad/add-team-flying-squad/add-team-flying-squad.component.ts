...
import { CenterMasterDDLDataModel, CenterObserverSearchModel, DeploymentDataModel } from '../../../Models/CenterObserverDataModel';
...
@Component({
  selector: 'app-add-team-flying-squad',
  standalone: false,
  templateUrl: './add-team-flying-squad.component.html',
  styleUrl: './add-team-flying-squad.component.css'
})

export class AddTeamFlyingSquadComponent {
  ...
  filterData: any[] = [];
  getFlyingSquadTeamMemberData: any[] = [];
  ...
  postTeamDeploymentFlyingSquadForm() {
    ...
    debugger
    setTimeout(() => {
      if (this.getFlyingSquadTeamMemberData.length === 0) {
        this.toastr.warning('Please add the team member, then try again.');
        return;
      }
  ...
  downloadPDF(team: any) {
    try {
      ...
      this.reportService.GetFlyingSquadDutyOrder(obj)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
  ...
  GetFlyingSquadReports(team: any) {
    try {
      ...
      this.reportService.GetFlyingSquadOrderReports(obj)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
            this.DownloadFile(data.Data, 'file download');
  ...
  async getFlyingSquadTeamMember() {
    try {
      this.getFlyingSquadTeamMemberData = [];
      let obj = {
        TeamID: this.TeamDeploymentFlyingSquadForm.value.TeamID,
        EndTermId: this.sSOLoginDataModel.EndTermID,
        DepartmentID: this.sSOLoginDataModel.DepartmentID,
        CourseTypeID: this.sSOLoginDataModel.Eng_NonEng,
      };
      await this.flyingSquadService.getFlyingSquad(obj)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data['Data']));
          this.getFlyingSquadTeamMemberData = data; 
        }, error => console.error(error));
