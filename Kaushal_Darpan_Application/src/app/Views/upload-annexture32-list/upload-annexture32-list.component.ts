...
import { AnnexureDataModel } from '../../Models/PreExamStudentDataModel';
import { Annexture32DataModel } from '../../Models/BTER/UploadAnnextureDataModel';

@Component({
  selector: 'app-upload-annexture32-list',
  standalone: false,
  templateUrl: './upload-annexture32-list.component.html',
  styleUrl: './upload-annexture32-list.component.css'
})
export class UploadAnnexture32ListComponent {
  ...
  public searchRequest = new Annexture32DataModel();
  ...
  async getExaminerData() {
    ...
    try {
      await this.ScholarshipService.GetUploadAnnexture32(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
