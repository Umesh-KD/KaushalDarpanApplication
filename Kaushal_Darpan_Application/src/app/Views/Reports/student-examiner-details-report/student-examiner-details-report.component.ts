...
import { GetRevaluationStudentDetailReport, GetStudentExaminerDetailReport } from '../../../Models/GenerateAdmitCardDataModel';
...
@Component({
  selector: 'app-student-examiner-details-report',
  templateUrl: './student-examiner-details-report.component.html',
  styleUrls: ['./student-examiner-details-report.component.css'],
  standalone: false
})
export class StudentExaminerDetailReportsComponent implements OnInit {

  // Data binding for College Wise Reports
  public StudentExaminerDetailReportsModellList: GetStudentExaminerDetailReport[] = [];
  ...
  exportToExcel(): void {
    ...
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.StudentExaminerDetailReportsModellList);
    // Create a new Excel workbook this.PreExamStudentData
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');

    // Export the Excel file
    XLSX.writeFile(wb, 'StudentExaminerDetailReport.xlsx');
  }
  ...
  async GetAllData() {
    ...
    this.StudentExaminerDetailReportsModellList = [];
    try {
      this.loaderService.requestStarted();
      await this.reportService.GetStudentExaminerDetailReport(requestData)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State === EnumStatus.Success) {
            this.StudentExaminerDetailReportsModellList = data['Data'];
            this.exportToExcel();
            this.buildDynamicColumns();
            this.dataSource = new MatTableDataSource(this.StudentExaminerDetailReportsModellList);
            this.dataSource.sort = this.sort;
            this.totalRecords = this.StudentExaminerDetailReportsModellList.length;
  ...
  buildDynamicColumns(): void {
    if (!this.StudentExaminerDetailReportsModellList.length) return;

    const sampleItem = this.StudentExaminerDetailReportsModellList[0];
  ...
  updateTable(): void {
    ...
    this.dataSource.data = this.StudentExaminerDetailReportsModellList.slice(startIndex, adjustedEndIndex);
