...
import { GetRMIFailStudentReport } from '../../../Models/GenerateAdmitCardDataModel';
...
@Component({
  selector: 'app-rmi-fail-student-report',
  standalone: false,
  templateUrl: './rmi-fail-student-report.component.html',
  styleUrl: './rmi-fail-student-report.component.css'
})
export class RMIFailStudentReportComponent {

  public RMIStudentFailReportList: GetRMIFailStudentReport[] = [];
  public searchRequest = new GetRMIFailStudentReport();
  ...
  dataSource: MatTableDataSource<GetRMIFailStudentReport> = new MatTableDataSource();
  ...
  exportToExcel() {
    ...
    this.RMIStudentFailReportList.forEach(row => {
    ...
    const formattedData = this.RMIStudentFailReportList.map((row, index) => {
    ...
    const fileName = `RMIStudentFailReport_Class.xlsx`;
  ...
  async GetAllData() {
    ...
    this.RMIStudentFailReportList = [];
    try {
      this.loaderService.requestStarted();
      await this.reportService.GetRMIFailStudentReport(requestData)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State === EnumStatus.Success) {
            this.RMIStudentFailReportList = data['Data'];
            this.exportToExcel();
            this.buildDynamicColumns();
            this.dataSource = new MatTableDataSource(this.RMIStudentFailReportList);
            this.dataSource.sort = this.sort;
            this.totalRecords = this.RMIStudentFailReportList.length;
  ...
  buildDynamicColumns() {
    if (this.RMIStudentFailReportList.length > 0) {
      const firstRow = this.RMIStudentFailReportList[0];
  ...
  updateTable(): void {
    ...
    this.dataSource.data = this.RMIStudentFailReportList.slice(startIndex, adjustedEndIndex);
