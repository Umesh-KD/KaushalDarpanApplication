...
@Component({
  selector: 'app-bter-bridge-course-reports',
  templateUrl: './bter-bridge-course-reports.component.html',
  styleUrls: ['./bter-bridge-course-reports.component.css'],
    standalone: false
})
export class BridgeCourseReportsComponent implements OnInit {
  ...
  async Download(): Promise<void> {
    try {
      ...
      const data = await this.reportService.BterBridgeCoruseReportDownload1(requestData);
      if (data && data.Data) {   
        this.downloadBase64PDF(data.Data, 'BridgeCourse.pdf');
      } else {
