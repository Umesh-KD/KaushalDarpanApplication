...
import { GetTheoryFailStudentReport } from '../../../Models/GenerateAdmitCardDataModel';
...
@Component({
  selector: 'app-theory-paper-fail-student',
  standalone: false,
  templateUrl: './theory-paper-fail-student.component.html',
  styleUrl: './theory-paper-fail-student.component.css'
})
export class TheoryFailStudentReportComponent {

  public TheoryFailStudentReportList: GetTheoryFailStudentReport[] = [];
  public searchRequest = new GetTheoryFailStudentReport();
  ...
  dataSource: MatTableDataSource<GetTheoryFailStudentReport> = new MatTableDataSource();
  ...
  exportToExcel() {
    ...
    const orderedColumns = ['SNo', 'RollNo','EnrollmentNo','Name', 'InstituteCode'];

    const allKeys = new Set<string>();
    this.TheoryFailStudentReportList.forEach(row => {
    ...
    const formattedData = this.TheoryFailStudentReportList.map((row, index) => {
    ...
    const fileName = `TheoryStudentFailReport_Class.xlsx`;
  ...
  async GetAllData() {
    ...
    if (this.filterForm?.value.selectedSemester == 0) {
      this.toastr.error("Please Select Semester First");
      return;
    }
    let requestData: any = {
    ...
    this.TheoryFailStudentReportList = [];
    try {
      this.loaderService.requestStarted();
      await this.reportService.GetTheoryFailStudentReport(requestData)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State === EnumStatus.Success) {
            this.TheoryFailStudentReportList = data['Data'];
            this.exportToExcel();
            this.buildDynamicColumns();
            this.dataSource = new MatTableDataSource(this.TheoryFailStudentReportList);
            this.dataSource.sort = this.sort;
            this.totalRecords = this.TheoryFailStudentReportList.length;
  ...
  buildDynamicColumns() {  
    if (this.TheoryFailStudentReportList.length > 0) {
      const firstRow = this.TheoryFailStudentReportList[0];
      this.displayedColumns = [];

      this.displayedColumns.push('Name', 'RollNo', 'EnrollmentNo', 'InstituteCode');
  ...
  updateTable(): void {
    ...
    this.dataSource.data = this.TheoryFailStudentReportList.slice(startIndex, adjustedEndIndex);
