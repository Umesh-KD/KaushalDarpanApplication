...
import { DTEApplicationDashboardModel } from '../../../Models/StudentCenteredActivitesModel';
import { ReportService } from '../../../Services/Report/report.service';

@Component({
  selector: 'app-CenterSuperintendentStudentReport',
  templateUrl: './CenterSuperintendentStudentReport.component.html',
  styleUrls: ['./CenterSuperintendentStudentReport.component.css'],
    standalone: false
})
export class CenterSuperintendentStudentReportComponent implements OnInit {
  ...
  requestDTEApplicationDashboardModel = new DTEApplicationDashboardModel();
  ...
  constructor(private commonMasterService: CommonFunctionService, private Router: Router, private Board_UniversityMasterService: Board_UniversityMasterService, private toastr: ToastrService,
    private loaderService: LoaderService, private router: ActivatedRoute, private routers: Router, private modalService: NgbModal, private Swal2: SweetAlert2, private ReportService: ReportService) {
  }
  ...
  async GetAllData() {
    ...
    try {
      this.requestDTEApplicationDashboardModel.EndTermID = this.sSOLoginDataModel.EndTermID,
        this.requestDTEApplicationDashboardModel.DepartmentID = this.sSOLoginDataModel.DepartmentID,
        this.requestDTEApplicationDashboardModel.Eng_NonEng = this.sSOLoginDataModel.Eng_NonEng,
        this.requestDTEApplicationDashboardModel.UserID = this.sSOLoginDataModel.UserID,
        this.requestDTEApplicationDashboardModel.RoleID = this.sSOLoginDataModel.RoleID,
        this.requestDTEApplicationDashboardModel.Status = 1,
        this.requestDTEApplicationDashboardModel.InstituteID = this.sSOLoginDataModel.InstituteID

      await this.ReportService.GetCenterSuperintendentStudentReport(this.requestDTEApplicationDashboardModel)
        .then((data: any) => {
          this.filteredDataNew = data['Data'];
         
          console.log('test data filteredData', this.filteredDataNew)
  ...
  exportToExcel(): void {
    // Map only the required columns and define their order
    const exportData = this.filteredDataNew.map((element: any) => ({
      EnrollmentNo: element.EnrollmentNo,
      StudentName: element.StudentName,
      MobileNo: element.MobileNo,
      DOB: element.DOB,
      Email: element.Email,
      CCCode: element.CCCode,
      InstituteNameEnglish: element.InstituteNameEnglish,
      SubjectCode: element.SubjectCode,
      BrancCode: element.BrancCode,
      ExamShift: element.ExamShift,
    ...
    XLSX.writeFile(wb, 'GetCenterSuperintendentStudentReport.xlsx');
  }


  //exportToExcel(): void {
  //  const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.filteredDataNew);
  //  const wb: XLSX.WorkBook = XLSX.utils.book_new();
  //  XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
  //  XLSX.writeFile(wb, 'GetCenterSuperintendentStudentReport.xlsx');
  //}
