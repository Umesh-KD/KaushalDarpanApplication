...
import { VerifierApiDataModel, VerifierDataModel } from '../../../Models/VerifierDataModel';
...
export class AddVerifierComponent implements OnInit {
  ...
  async SaveVerifierData() {
    ...
    try {
      ...
      this.request.DepartmentID = this.SSOLoginDataModel.DepartmentID;
      if (this.request.DepartmentID == EnumDepartment.ITI) {
        this.request.RoleID = EnumRole.ITI_Verifier
      } else {
        this.request.RoleID = EnumRole.BTERVerifier
      }
      ...
      await this.verifierService.SaveVerifierData(this.request).then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
        if (data.State == EnumStatus.Success)
        {
            this.toastr.success(data.Message)
            this.ResetControl()
            this.router.navigate(['/verifiers']);
          } else {
