...
@Component({
  selector: 'app-principal-dispatch-group',
  standalone: false,
  templateUrl: './principal-dispatch-group.component.html',
  styleUrl: './principal-dispatch-group.component.css'
})
export class PrincipalDispatchGroupComponent {
  ...
  public handoverFileName: string = '';
...
/*    this.getgroupteacherData()*/
  ...
  async DownloadAckReportPri() {
    ...
    try {
      await this.DispatchService.DownloadAckReportPri(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
  ...
  async getgroupteacherData() {
    ...
    if (this.searchRequest.Status == this._DispatchDDlValue.AssigntoCollageFromBter) {
      this.searchRequest.ExaminerStatus = 0;
    }
    if (this.searchRequest.Status == this._DispatchDDlValue.VerifyByExaminer) {
      this.searchRequest.ExaminerStatus = 20;
    }
    if (this.searchRequest.Status == this._DispatchDDlValue.HandOverToExaminerByPrincipal) {
      this.searchRequest.ExaminerStatus = 1;
    }
    if (this.searchRequest.Status == this._DispatchDDlValue.HandOverToPrincipalByExaminer) {
      this.searchRequest.ExaminerStatus = 33;
    }
    try {
      await this.DispatchService.getgroupteacherData(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
    ...
    try {
      ...
      await this.commonMasterService.GetddlCenterDownloadOrReceived(Type)
        .then((data: any) => {
          ...
          this.StatusTypelist = this.StatusTypelist.filter((item: any) => item.ID == 20 || item.ID == 32 || item.ID == 33);
  ...
  async DownloadDispatchGroupCertificate(ID: number, StaffID:number) {
    try {
      ...
      await this.reportService.DownloadDispatchGroupCertificate(ID, StaffID ,this.sSOLoginDataModel.DepartmentID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
  ...
  async CheckOtpAfterSave() {
    ...
    //Selected.forEach((e: any) => { e.Status = 1, e.ModifyBy = this.sSOLoginDataModel.UserID })
    this.loaderService.requestStarted();
          ...
          .then((data: any) => {
            ...
            if (this.State == EnumStatus.Success) {
              if (Type == 'Photo') {
                ...
                this.handoverFileName = this.UpdateFileHandover.FileName;
              }
              //else if (Type == "Sign") {
              //  this.request.Dis_CompanyName = data['Data'][0]["Dis_FileName"];
  ...
  async SaveFileData() {
    ...
    try {
      ...
      this.loaderService.requestStarted();
      this.UpdateFileHandover.FileName = this.handoverFileName;
      try {
