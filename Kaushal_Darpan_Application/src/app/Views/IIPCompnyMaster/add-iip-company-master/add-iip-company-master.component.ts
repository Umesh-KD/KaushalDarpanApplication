...
@Component({
  selector: 'app-add-iip-company-master',
  standalone:false,
  templateUrl: './add-iip-company-master.component.html',
  styleUrl: './add-iip-company-master.component.css'
})
export class AddIipCompanyMasterComponent implements OnInit {
  ...
  validators: this.atLeastOneCheckboxCheckedValidator
    ...
    atLeastOneCheckboxCheckedValidator(group: FormGroup) {
  const isIIP = group.get('ISIIP')?.value;
  const isPlacement = group.get('ISPlacement')?.value;

  return isIIP || isPlacement
    ? null
