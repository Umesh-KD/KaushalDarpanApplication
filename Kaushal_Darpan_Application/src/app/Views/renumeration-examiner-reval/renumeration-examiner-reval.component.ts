...
import { RenumerationExaminerRequestModel, RenumerationExaminerModel, TrackStatusDataModel, RenumerationExaminerPDFModel } from '../../Models/RenumerationExaminerModel';
import { SSOLoginDataModel } from '../../Models/SSOLoginDataModel';
import { LoaderService } from '../../Services/Loader/loader.service';
import { ReportService } from '../../Services/Report/report.service';
import { RenumerationExaminerRevalService } from '../../Services/renumeration-examiner-reval/renumeration-examiner-reval.service';

@Component({
  selector: 'app-renumeration-examiner-reval',
  standalone: false,
  templateUrl: './renumeration-examiner-reval.component.html',
  styleUrl: './renumeration-examiner-reval.component.css'
})
export class RenumerationExaminerRevalComponent implements OnInit {
  public Message: any = [];
  ...
  constructor(private commonMasterService: CommonFunctionService,
    ...
    private renumerationExaminerRevalService: RenumerationExaminerRevalService,
  ...
  async GetTrackStatusData() {
    ...
    try {
      ...
      //call
      await this.renumerationExaminerRevalService.GetTrackStatusData(this.RenumerationExaminerRequest)
        .then(async (data: any) => {
  ...
  async GetAllData() {
    try {
      ...
      this.RenumerationExaminerRequest.RoleID = this.sSOLoginDataModel.RoleID;
      //call
      await this.renumerationExaminerRevalService.GetAllData(this.RenumerationExaminerRequest)
        .then(async (data: any) => {
          if (data.State == EnumStatus.Error) {
            this.toastr.error(data.Message);
            console.log(data.ErrorMessage);
  ...
  async GenerateAndViewPdf(ExaminerID: number) {
    try {
      ...
      //call
      await this.reportService.GenerateAndViewPdf_Reval(this.RenumerationExaminerRequest)
        .then((response: any) => {
          ...
          if (contentType === 'application/pdf') {//1.
            ...
            let fileName = 'RemunerationReval.pdf';
  ...
  async SavePDFSubmitAndForwardToJD() {
    try {
      this.isSubmitted = true;
      this.RenumerationExaminerRequest.RoleID = this.sSOLoginDataModel.RoleID;
      await this.reportService.SavePDFSubmitAndForwardToJD_Reval(this.RenumerationExaminerRequest)
        .then(async (data: any) => {
          if (data.State == EnumStatus.Error) {
            this.toastr.error(data.Message);
            console.log(data.ErrorMessage);
