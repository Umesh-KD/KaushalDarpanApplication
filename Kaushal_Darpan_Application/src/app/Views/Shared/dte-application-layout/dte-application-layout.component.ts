...
import { EnumConfigurationType, EnumDepartment, EnumRole, EnumStatus, GlobalConstants } from '../../../Common/GlobalConstants';
...
@Component({
  selector: 'dte-application-layout-page',
  templateUrl: './dte-application-layout.component.html',
  styleUrls: ['./dte-application-layout.component.css'],
    standalone: false
})
export class DTEApplicationLayoutComponent implements OnInit {
  ...
  constructor(private breakpointObserver: BreakpointObserver,  private el: ElementRef, @Inject(DOCUMENT) private document: Document, @Inject(PLATFORM_ID) private platformId: Object, private router: Router, private loaderService: LoaderService,
    ...
    private cookieService: CookieService, private menuService: MenuService, private http: HttpClient, private appsettingConfig: AppsettingService, private renderer: Renderer2, private dateMasterService: DateConfigService) {
    location.onPopState(() => {
  ...
  async GetCurrentAdmissionSession() {
    try {
      await this.commonFunctionService.GetCurrentAdmissionSession(this.DepartmentID)
        .then(async (data: any) => {
  ...
  async ngOnInit() {
    ...
    this.DepartmentID = 1;// this.sSOLoginDataModel.DepartmentID;
