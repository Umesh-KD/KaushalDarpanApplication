...
@Component({
  selector: 'app-iti-10th-trade-wise-admission-status',
  standalone: false,
  templateUrl: './iti-10th-trade-wise-admission-status.component.html',
  styleUrl: './iti-10th-trade-wise-admission-status.component.css'
})
export class Iti10ThTradeWiseAdmissionStatusComponent implements OnInit {
  ...
  async BtnSearch() {
    ...
    if (this.searchRequest.CourseTypeID == 0  && this.searchRequest.ITITradeID == 0 && this.searchRequest.TradeCode == '') {
  ...
  async GetList() {
    try {
     ...
     // this.searchRequest.EndTermID = this.sSOLoginDataModel.EndTermID;
      await this.ReportServices.GetITITradeWiseAdmissionStatusDataList(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  exportToExcel(): void {
    ...
    XLSX.writeFile(wb, '10th-trade-wise-admission-status.xlsx');
  }
