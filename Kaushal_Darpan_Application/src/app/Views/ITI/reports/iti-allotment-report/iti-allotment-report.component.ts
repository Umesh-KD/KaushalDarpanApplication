...
@Component({
  selector: 'app-iti-allotment-report',
  standalone: false,
  templateUrl: './iti-allotment-report.component.html',
  styleUrl: './iti-allotment-report.component.css'
})
export class ITIAllotmentReportComponent {
  ...
  async ngOnInit() {
    ...
    this.searchRequest.TradeLevelID = Number(this.activatedRoute.snapshot.paramMap.get('iid') ?? 0)
    this.sSOLoginDataModel = await JSON.parse(String(localStorage.getItem('SSOLoginUser')));
   
    await this.GetCollegeNames();
    await this.GetITIAllotmentReportList();
    await this.GetTradeListDDL();

  }

  async GetITIAllotmentReportList() {
    ...
    try {
      ...
      await this.ReportService.ITIAllotmentReport(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.ITIAllotmentList = data?.Data?.Table || [];
          this.totalInTableRecord = this.ITIAllotmentList.length;
          this.loadInTable();
          console.log(this.ITIAllotmentList);
  ...
  ITIAllotmentReportExportToExcel() {
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.ITIAllotmentList);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    const fileName = `ITIAllotmentReportList_Class.xlsx`;
  ...
  Reset() {
    ...
    this.GetITIAllotmentReportList();
  }
  ...
  updateInTablePaginatedData() {
    ...
    this.paginatedInTableData = [...this.ITIAllotmentList].slice(this.startInTableIndex, this.endInTableIndex);
    this.loaderService.requestEnded();
  }
  previousInTablePage() {
