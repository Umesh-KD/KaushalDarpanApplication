...
@Component({
  selector: 'app-iti-student-seat-allotment-report',
  standalone: false,
  templateUrl: './iti-student-seat-allotment-report.component.html',
  styleUrl: './iti-student-seat-allotment-report.component.css'
})
export class ItiStudentSeatAllotmentReportComponent implements OnInit {
  ...
  async GetList() {
    try {
      ...
      await this.ReportServices.GetITIStudentSeatAllotmentDataList(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async BtnSearch() {
    ...
    if (this.searchRequest.TradeLevelID == 0 && this.searchRequest.ITICollegeID == 0 && this.searchRequest.ITITradeID == 0 && this.searchRequest.ITICode == '' && this.searchRequest.TradeCode == '') {
      this.toastr.error("Please select at least one filter.");
      return;
    }
    await this.GetList();
  ...
  exportToExcel(): void {
    ...
    XLSX.writeFile(wb, 'Student-Seat-Allotment report.xlsx');
  }
