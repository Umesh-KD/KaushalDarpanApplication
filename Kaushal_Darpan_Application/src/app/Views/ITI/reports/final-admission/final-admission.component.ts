...
import { FinalAdmissionListRequestModel, ZoneDistrictSeatUtilizationRequestModel } from '../../../../Models/TheoryMarksDataModels';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-final-admission',
  standalone: false,
  templateUrl: './final-admission.component.html',
  styleUrl: './final-admission.component.css'
})
export class FinalAdmissionComponent {
  public isFormSubmitted: boolean = false;
  public isLoading: boolean = false;
  public requestModel = new ZoneDistrictSeatUtilizationRequestModel();
  ...
  async ngOnInit() {
    ...
    await this.GetFinalAdmissionList();

  }


  async GetFinalAdmissionList() {
    this.requestModel = {
    ...
    try {
      ...
      await this.ReportService.getZoneFinalAdmissionList(this.requestModel)
        .then((data: any) => {
  ...
  //Search Function for District Wise and Division(Zone) Wise 
  ...
  // for download data in Excel Sheet
  exportToExcel() {
    let className = '';
    ...
    const fileName = `FinalAdmissionList_${className}_Class.xlsx`;
