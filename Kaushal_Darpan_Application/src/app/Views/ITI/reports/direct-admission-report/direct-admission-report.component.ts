...
import { DirectAdmissionReportRequestModel } from '../../../../Models/TheoryMarksDataModels';
import * as XLSX from 'xlsx';
import { ItiTradeSearchModel } from '../../../../Models/CommonMasterDataModel';

@Component({
  selector: 'app-direct-admission-report',
  standalone: false,
  templateUrl: './direct-admission-report.component.html',
  styleUrl: './direct-admission-report.component.css'
})
export class DirectAdmissionReportComponent {
  public isFormSubmitted: boolean = false;
  public isLoading: boolean = false;
  public searchRequest = new DirectAdmissionReportRequestModel();
  public AllListConfiguration: ListAllotmentConfigurationModel[] = [];
  public State: number = 0;
  public Message: string = '';
  public ErrorMessage: string = '';
  public sSOLoginDataModel = new SSOLoginDataModel();
  public DirectAdmissionList: any[] = [];
  ...
  async ngOnInit() {
    ...
    await this.GetDirectAdmissionList();
    await this.GetTradeListDDL();

  }

  async GetDirectAdmissionList() {

    this.searchRequest.AcademicYearID = this.sSOLoginDataModel.FinancialYearID;

    try {
      this.loaderService.requestStarted();
      await this.ReportService.DirectAdmissionReport(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.DirectAdmissionList = data?.Data?.Table || [];
          this.totalInTableRecord = this.DirectAdmissionList.length;
          this.loadInTable();
          console.log(this.DirectAdmissionList);
  ...
  DirectAdmissionReportExportToExcel() {
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.DirectAdmissionList);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    const fileName = `DirectAdmissionList_Class.xlsx`;
  ...
  updateInTablePaginatedData() {
    ...
    this.paginatedInTableData = [...this.DirectAdmissionList].slice(this.startInTableIndex, this.endInTableIndex);
    this.loaderService.requestEnded();
  }
  previousInTablePage() {
