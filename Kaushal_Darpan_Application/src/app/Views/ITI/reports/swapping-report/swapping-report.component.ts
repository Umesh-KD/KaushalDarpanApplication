...
import { SwappingReportRequestModel } from '../../../../Models/TheoryMarksDataModels';
import * as XLSX from 'xlsx';
import { ItiTradeSearchModel } from '../../../../Models/CommonMasterDataModel';

@Component({
  selector: 'app-swapping-report',
  standalone: false,
  templateUrl: './swapping-report.component.html',
  styleUrl: './swapping-report.component.css'
})
export class SwappingReportComponent {
  public isFormSubmitted: boolean = false;
  public isLoading: boolean = false;
  public searchRequest = new SwappingReportRequestModel();
  public AllListConfiguration: ListAllotmentConfigurationModel[] = [];
  public State: number = 0;
  public Message: string = '';
  public ErrorMessage: string = '';
  public sSOLoginDataModel = new SSOLoginDataModel();
  public swappingList: any[] = [];
  ...
  async ngOnInit() {
    ...
    await this.GetSwappingList();
    await this.GetTradeListDDL();

  }

  async GetSwappingList() {
    this.searchRequest.AcademicYearID = this.sSOLoginDataModel.FinancialYearID  
   
    try {
      this.loaderService.requestStarted();
      await this.ReportService.GetSwappingReport(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.swappingList = data?.Data?.Table || [];
          this.totalInTableRecord = this.swappingList.length;
          this.loadInTable();
          console.log(this.swappingList);
  ...
  public Reset() {
    ...
    this.GetSwappingList();
  }

  SwappingReportExportToExcel() {
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.swappingList);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    const fileName = `SwappingReportList_Class.xlsx`;
  ...
  updateInTablePaginatedData() {
    ...
    this.paginatedInTableData = [...this.swappingList].slice(this.startInTableIndex, this.endInTableIndex);
    this.loaderService.requestEnded();
  }
  previousInTablePage() {
