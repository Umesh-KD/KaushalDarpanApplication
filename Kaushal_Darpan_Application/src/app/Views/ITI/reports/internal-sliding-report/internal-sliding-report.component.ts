...
import { InternalSlidingReportRequestModel } from '../../../../Models/TheoryMarksDataModels';
import * as XLSX from 'xlsx';
import { ItiTradeSearchModel } from '../../../../Models/CommonMasterDataModel';

@Component({
  selector: 'app-internal-sliding-report',
  standalone: false,
  templateUrl: './internal-sliding-report.component.html',
  styleUrl: './internal-sliding-report.component.css'
})
export class InternalSlidingReportComponent {
  public isFormSubmitted: boolean = false;
  public isLoading: boolean = false;
  public searchRequest = new InternalSlidingReportRequestModel();
  ...
  async ngOnInit() {
    ...
    await this.GetInternalSlidingList();
    await this.GetTradeListDDL();

  }

  async GetInternalSlidingList() {

    this.searchRequest.AcademicYearID = this.sSOLoginDataModel.FinancialYearID  
   
    try {
      this.loaderService.requestStarted();
      await this.ReportService.GetInternalSlidingReport(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.internalSlidingList = data?.Data?.Table || [];
          this.totalInTableRecord = this.internalSlidingList.length;
          this.loadInTable();
          console.log(this.internalSlidingList);
  ...
  public Reset() {
    ...
    this.GetInternalSlidingList();
  }

  InternalSlidingReportExportToExcel() {
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.internalSlidingList);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    const fileName = `InternalSlidingReportList_Class.xlsx`;
  ...
  updateInTablePaginatedData() {
    ...
    this.paginatedInTableData = [...this.internalSlidingList].slice(this.startInTableIndex, this.endInTableIndex);
    this.loaderService.requestEnded();
  }
  previousInTablePage() {
