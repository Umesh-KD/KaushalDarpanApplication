...
@Component({
  selector: 'app-student-seat-withdraw-report',
  standalone: false,
  templateUrl: './student-seat-withdraw-report.component.html',
  styleUrl: './student-seat-withdraw-report.component.css'
})
export class StudentSeatWithdrawReportComponent {
  ...
  async BtnSearch() {
    ...
    if (this.searchRequest.TradeLevelID == 0 && this.searchRequest.ITICollegeID == 0 && this.searchRequest.ITITradeID == 0 && this.searchRequest.ITICode == '' &&  this.searchRequest.TradeCode?.trim() == ''
      /*this.searchRequest.TradeCode == ''*/) {
      this.toastr.error("Please select at least one filter.");
      return;
    }
  ...
  exportToExcel(): void {
    ...
    XLSX.writeFile(wb, 'Student-Seat-Withdraw-report.xlsx');
  }
