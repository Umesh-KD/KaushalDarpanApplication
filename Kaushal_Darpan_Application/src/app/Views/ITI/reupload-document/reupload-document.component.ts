...
@Component({
  selector: 'app-reupload-document',
  standalone: false,
  templateUrl: './reupload-document.component.html',
  styleUrl: './reupload-document.component.css'
})
export class ReuploadDocumentComponent {
  ...
  public filteredDocumentDetails2: AllotmentDocumentModel[] = []
  ...
  async GetById() {
    ...
    try {
      ...
      await this.StudentsJoiningStatusMarksService.GetCorrectDocumentdata(this.searchRequest)
        .then((data: any) => {
          ...
          if (data['Data'] != null) {
            ...
            if (this.request?.AllotmentDocumentModel) {
              ...
              this.filteredDocumentDetails2 = this.request.AllotmentDocumentModel.filter((x) => x.DocumentStatus ==false );
  ...
  async SaveReporting() {
    ...
    //  this.request.JoiningStatus = 'J'
    //}
    if (this.documentDetailsService.HasRequiredDocument(this.request.AllotmentDocumentModel)) {
      return;
    }

    this.request.AllotmentDocumentModel = this.request.AllotmentDocumentModel.filter((e) => e.DocumentStatus == false)
    this.request.AllotmentDocumentModel.forEach((e) => e.DocumentStatus = true)
    try {
      await this.StudentsJoiningStatusMarksService.SaveCorrectDocument(this.request)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
            this.toastr.success(data.Message)
  ...
  async OnRoute() {
    if (this.TradeLevel == 10) {
      this.router.navigate(['ReuploadDocumentList10th/10'])
    } else if (this.TradeLevel == 8) {
      this.router.navigate(['ReuploadDocumentList8th/8'])
    } else if (this.TradeLevel == 12) {
      this.router.navigate(['ReuploadDocumentList12th/12'])
    }
  }
      ...
      await this.documentDetailsService.UploadDocument(event, uploadModel)
        .then((data: any) => {
          ...
          if (data.State == EnumStatus.Success) {
            ...
            const index = this.request.AllotmentDocumentModel.findIndex((x: any) => x.DocumentMasterID == item.DocumentMasterID && x.DocumentDetailsID == item.DocumentDetailsID);
            if (index !== -1) {
              this.request.AllotmentDocumentModel[index].FileName = data.Data[0].FileName;
              this.request.AllotmentDocumentModel[index].Dis_FileName = data.Data[0].Dis_FileName;
            }
            console.log(this.filteredDocumentDetails2)
            //reset file type
      ...
      await this.documentDetailsService.DeleteDocument(deleteModel)
        .then((data: any) => {
          ...
          if (data.State != EnumStatus.Error) {
            ...
            const index = this.request.AllotmentDocumentModel.findIndex((x: any) => x.DocumentMasterID == item.DocumentMasterID && x.DocumentDetailsID == item.DocumentDetailsID);
            if (index !== -1) {
              this.request.AllotmentDocumentModel[index].FileName = '';
              this.request.AllotmentDocumentModel[index].Dis_FileName = '';
            }
            console.log(this.filteredDocumentDetails2)
          }
          if (data.State == EnumStatus.Error) {
