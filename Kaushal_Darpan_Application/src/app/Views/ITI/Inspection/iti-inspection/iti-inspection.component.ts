...
@Component({
  selector: 'app-iti-inspection',
  standalone: false,
  templateUrl: './iti-inspection.component.html',
  styleUrl: './iti-inspection.component.css'
})
export class ITIInspectionComponent {
 ...
 // private modalService = inject(NgbModal);
  ...
  async GetAllData () {
    try {
      ...
      this.searchRequest.UserID = this.sSOLoginDataModel.UserID
      this.searchRequest.LevelId = this.sSOLoginDataModel.LevelId
      await this.itiInspectionService.GetAllData(this.searchRequest).then((data: any) => {
  ...
  async openModalGenerateOTP(content: any, id : number) {
    ...
    //this.request = item;
    await this.SendOTP();
    this.InspectionTeamID = id; // Store the ID of the inspection team for later use
  }
  ...
  async DownloadPdf(FileName: string) {
    ...
    this.http.get(fileUrl, { responseType: 'blob' }).subscribe((blob: any) => {
      ...
      downloadLink.download = "InspectionDutyOrder.pdf"; // Set the desired file name
      downloadLink.click();
  ...
  exportToExcel(): void {
    const unwantedColumns = ['ActiveStatus', 'DeleteStatus', 'CreatedBy', 'ModifyBy', 'ModifyDate', 'IPAddress', 'InspectionTeamID','DeployTeamStatus'];
    const filteredData = this.InspectionData.map((item:any) => {
    ...
    const fileName = `InspectionDetails_${dateStr}.xlsx`;
  ...
  //  const wb: XLSX.WorkBook = XLSX.utils.book_new();
  //  XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
  //  XLSX.writeFile(wb, `InspectionDetails_${new Date().toISOString().split('T')[0]}.xlsx`);
  //}
  ...
  exportToPDF() {
    ...
    doc.setFontSize(9);
    doc.text(
      `Total Records : ${this.InspectionData.length}`,
      pageWidth - 20,
      10,
      { align: 'right' }
    );
    ...
    const body = this.InspectionData.map((row: any, index: number) => [
      index + 1,
      //`${row.Code || ''} (${row.Name || ''})`,  //concate in one column
      row.InspectionTeamName || '',
      row.TeamTypeName || '',
      row.DeploymentDateFrom || '',
      row.DeploymentDateTo || '',
      row.DeploymentStatus==1? 'Deployed':'Deploy Pending',
      //row.DgetCode || '',
      //row.campusName || '',
    ...
    doc.save('ITI Inspection List.pdf');
  }
