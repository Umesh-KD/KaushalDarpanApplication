...
@Component({
  ...
  standalone: false,
  templateUrl: './iti-inspection-report.component.html',
  styleUrl: './iti-inspection-report.component.css'
})
export class ITIInspectionReportComponent {
 ...
 // private modalService = inject(NgbModal);
  ...
  async GetAllData () {
    try {
      ...
      this.searchRequest.UserID = this.sSOLoginDataModel.UserID
      this.searchRequest.LevelId = this.sSOLoginDataModel.LevelId
      await this.itiInspectionService.GetAllInspectedData(this.searchRequest).then((data: any) => {
    ...
    if (this.OTP.length > 0) {
      if ((this.OTP == GlobalConstants.DefaultOTP) || (this.OTP == this.GeneratedOTP)) {
        ...
        try {
          ...
          try {
            await this.itiInspectionService.GenerateCOAnsweredReport(id).then((data: any) => {
