...
@Component({
    ...
    templateUrl: './document-details-form-tab.component.html',
    styleUrls: ['./document-details-form-tab.component.css'],
    standalone: false
})
export class DocumentDetailsFormTabComponent implements OnInit {
      ...
      await this.ItiApplicationFormService.GetApplicationDatabyID(this.searchRequest)
        .then(async (data: any) => {
          ...
          if (data['Data'] != null){ 
            ...
            await this.filteredDocumentDetails(2);
          }
        }, error => console.error(error));
    }
    catch (ex) { console.log(ex) }
