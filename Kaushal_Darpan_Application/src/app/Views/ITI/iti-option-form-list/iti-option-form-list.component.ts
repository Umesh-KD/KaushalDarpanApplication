...
@Component({
  selector: 'app-iti-option-form-list',
  templateUrl: './iti-option-form-list.component.html',
  styleUrls: ['./iti-option-form-list.component.css'],
  standalone: false
})
export class ItiOptionFormListComponent {
  ...
  async ngOnInit() {
    ...
    this.searchRequest.ManagementTypeId = Number(this.route.snapshot.paramMap.get('type'));
    this.searchRequest.TradeLevelId = Number(this.route.snapshot.paramMap.get('trade'));
  ...
  async GetAllData(i: any) {
    ...
    try {
      ...
      await this.itiApplicationService.GetItiOptionFormData(this.searchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
