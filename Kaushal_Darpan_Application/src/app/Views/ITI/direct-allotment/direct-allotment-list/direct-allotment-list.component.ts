...
@Component({
  selector: 'app-direct-allotment-list',
  standalone: false,

  templateUrl: './direct-allotment-list.component.html',
  styleUrl: './direct-allotment-list.component.css'
})
export class DirectAllotmentListComponent {
  ...
  async getIMCAllotedList() {
    try { 
      ...
      await this.allotmentService.GetAllData(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
    ...
    this.Swal2.Confirmation(confirmationMessage, async (result: any) => {
      if (result.isConfirmed) {
        ...
        try {
          ...
          this.request.InstituteID = this.sSOLoginDataModel.InstituteID;
          this.request.AcademicYearID = this.sSOLoginDataModel.FinancialYearID;
          await this.allotmentService.RevertAllotments(this.request)
            .then((data: any) => {
              data = JSON.parse(JSON.stringify(data));
              this.State = data['State'];
              this.Message = data['Message'];
  ...
  async GetAllotmentReceipt(AllotmentId: any) {
    ...
    try {
      this.loaderService.requestStarted();
      await this.allotmentService.GetDirectAdmissionReceipt(AllotmentId)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data && data.Data) {
