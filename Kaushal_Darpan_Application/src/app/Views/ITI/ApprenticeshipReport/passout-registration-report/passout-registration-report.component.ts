...
@Component({
  ...
  standalone: false,
  templateUrl: './passout-registration-report.component.html',
  styleUrl: './passout-registration-report.component.css'
})
export class PassoutRegistrationReportComponent {
  ...
  async saveData() {
    ...
    try {
      ...
      await this.ITITimeTableService.SavePassoutReport(this.request)
        .then((data: any) => {
          if (data.State == EnumStatus.Success) {
            this.toastr.success(data.Message)
            setTimeout(() => {
              this.router.navigate(['/PaasoutRegistrationReportList']);
            }, 1300);
  ...
  async SampleexportExcelDataYearly() {
    ...
    try {
      ...
      this.loaderService.requestStarted();
      await this.ITITimeTableService.SampleImportExcelFilePassout(this.searchRequest)
        .then((data: any) => {
...
""
  ...
  generateFileNameYearly(extension: string): string {
    ...
    return `ITIPassoutApprenticeship${timestamp}.${extension}`;
  }
 ...
 /*     this.ImportExcelFile(file);*/
  ...
  GoToReportEntryPage() {
    sessionStorage.setItem('PaasoutRegistrationReportListPKID', '0');
    this.router.navigate(['/PaasoutRegistrationReportList']);
  }
