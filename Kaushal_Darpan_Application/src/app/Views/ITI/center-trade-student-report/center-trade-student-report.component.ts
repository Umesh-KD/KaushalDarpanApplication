...
@Component({
  selector: 'app-center-trade-student-report',
  standalone: false,
  templateUrl: './center-trade-student-report.component.html',
  styleUrl: './center-trade-student-report.component.css'
})
export class CenterTradeStudentReportComponent {
  ...
  constructor(
    private TheoryMarksService: ReportService,
...
/*      this.searchRequest.CenterID = this.sSOLoginDataModel.InstituteID*/
      ...
      await this.TheoryMarksService.CenterWiseTradeStudentCount(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));

          this.TheoryMarksRptDataList = data['Data'];
          console.log(this.TheoryMarksRptDataList)
          //table feature load
          this.loadInTable();
  ...
  exportToExcel(): void {
    const unwantedColumns = [
      'StudentID', 'StudentExamID', 'StudentExamPaperMarksID', 'StudentExamPaperID', 'rowclass','SortOrder'
    ];
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'Center-Trade-wise-student.xlsx');
  }
