...
@Component({
  selector: 'app-iti-college-profile',
  standalone: false,
  templateUrl: './iti-college-profile.component.html',
  styleUrl: './iti-college-profile.component.css'
})

export class ITICollegeProfileComponent implements OnInit {
...
/*    await this.nonItiValidator();*/
    ...
    try {
      ...
      await this.collegeprofileService.GetByID(id)
        .then((data: any) => {
          ...
          this.request.StatusName = data['Data']["StatusName"];
  ...
  async saveData() {
    ...
    try {
      ...
      this.request.UpdatedBy = "College";
      //call
      await this.collegeprofileService.SaveData(this.request)
        .then((data: any) => {
          ...
          if (this.State == EnumStatus.Success) {
            ...
            this.routers.navigate(['/ItiPlanning'], { queryParams: { id: this.request.Id } });
