...
@Component({
  ...
  standalone: false,
  templateUrl: './add-intake-planning.component.html',
  styleUrl: './add-intake-planning.component.css'
})
export class AddIntakePlanningComponent {
...
/*    await this.GetOrderDetailsList();*/
  ...
  async GetCollegesListAll() {
    ...
    try {
      this.collegeSearchRequest.action = '_getAllData'
      await this.commonFunctionService.GetCommonMasterData('PlanningCollege').then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
        this.ItiCollegesListAll = data.Data
        console.log(this.ItiCollegesListAll, "ItiCollegesListAll")
      })
