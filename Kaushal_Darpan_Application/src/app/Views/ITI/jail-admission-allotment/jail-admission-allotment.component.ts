...
@Component({
  ...
  standalone: false,
  templateUrl: './jail-admission-allotment.component.html',
  styleUrl: './jail-admission-allotment.component.css'
})
export class JailAdmissionAllotmentComponent {
  ...
  async getIMCAllotedList() {
    try {
      ...
      await this.allotmentService.GetJailAllData(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
    ...
    this.Swal2.Confirmation(confirmationMessage, async (result: any) => {
      if (result.isConfirmed) {
        ...
        try {
          ...
          this.request.InstituteID = this.sSOLoginDataModel.InstituteID;
          this.request.AcademicYearID = this.sSOLoginDataModel.FinancialYearID;
          await this.allotmentService.RevertJailAllotments(this.request)
            .then((data: any) => {
              data = JSON.parse(JSON.stringify(data));
              this.State = data['State'];
              this.Message = data['Message'];
