...
@Component({
  selector: 'app-iti-admit-card',
  standalone: false,
  templateUrl: './iti-admit-card.component.html',
  styleUrl: './iti-admit-card.component.css'
})
...
/*    this.PDFType = Number(this.activeroute.snapshot.paramMap.get('id')??0)*/
  ...
  async GetAllData() {
    try {
      ...
      this.loaderService.requestStarted();
      //call
      await this.GetRollService.GetITIAdmit_RollListPdf(this.searchRequest).then(
        (data: any) => {
          data = JSON.parse(JSON.stringify(data));
