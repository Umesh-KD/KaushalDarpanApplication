...
@Component({
  selector: 'app-consumable-item-auction',
  standalone: false,
  templateUrl: './consumable-item-auction.component.html',
  styleUrl: './consumable-item-auction.component.css'
})
export class ConsumableItemAuctionComponent {
  ...
  //  for (let item of this.ItemMasterList) {
  //    item.Selected = isChecked;
  //  }
  //  console.log('id checked list===>',isChecked)
  //}
  updateSelectedRows() {
    this.request.RowsID = this.ItemMasterList
      .filter(x => x.Selected)
      .map(x => x.ItemId)
      .join(",");

    console.log("Selected IDs =>", this.request.RowsID);
  }

  checkUncheckAll(isChecked: boolean) {
    this.AllSelect = isChecked;

    this.ItemMasterList.forEach(item => {
      // Only allow selection if not already "Done"
      if (!(item.IsOption == '1' && item.AuctionStatus == '1')) {
        item.Selected = isChecked;
      }
    });
  ...
  async GetConsumableItemAuctionData() {
    try {
      this.loaderService.requestStarted();
      ...
      await this.itiInventoryService.GetConsumableItemAuctionData(this.Searchrequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.Message = data['Message'];
