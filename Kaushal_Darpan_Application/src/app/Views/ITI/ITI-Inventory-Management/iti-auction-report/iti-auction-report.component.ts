...
@Component({
  selector: 'app-iti-auction-report',
  standalone: false,
  templateUrl: './iti-auction-report.component.html',
  styleUrl: './iti-auction-report.component.css'
})

export class itiauctionreportComponent {
  ...
  async GetAllData() {
    try {
      ...
      await this.itiInventoryService.GetAllAuctionReport(this.Searchrequest)
        .then((data: any) => {
          debugger;
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
