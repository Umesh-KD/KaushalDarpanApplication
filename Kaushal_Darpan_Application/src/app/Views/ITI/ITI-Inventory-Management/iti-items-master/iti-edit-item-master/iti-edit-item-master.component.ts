...
@Component({
  selector: 'app-iti-edit-item-master',
  standalone: false,
  templateUrl: './iti-edit-item-master.component.html'
})
export class ITIEditItemMasterComponent {
...
this.itemsFormArray.get('items')?.valueChanges
  ...
  if (this.ItemId != null && this.ItemId != undefined && this.ItemId > 0) {
    await this.itiInventoryService.GetAllItemDetails(this.ItemId).then((data: any) => {
...
addItemsControls() {
  ...
  try {
    ...
    await this.itiInventoryService.UpdateItemData(itemsList).then((data: any) => {
  ...
  try {
    this.loaderService.requestStarted();
    await this.itiInventoryService.GetItemDetails(id)
      .then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
