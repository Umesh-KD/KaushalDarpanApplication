...
@Component({
  ...
  standalone: false,
  templateUrl: './iti-Apprenticeship.component.html',
  styleUrl: './iti-Apprenticeship.component.css'
})
export class ITIApprenticeshipComponent {
 ...
 // private modalService = inject(NgbModal);
  ...
  async GetAllData () {
    try {
      ...
      this.searchRequest.UserID = this.sSOLoginDataModel.UserID
      this.searchRequest.LevelId = this.sSOLoginDataModel.LevelId
      await this.itiApprenticeshipService.GetAllData(this.searchRequest).then((data: any) => {
        debugger;
  ...
  async openModalGenerateOTP(content: any, id: number) {
    ...
    //this.request = item;
    await this.SendOTP();
    this.ApprenticeshipTeamID = id; // Store the ID of the Apprenticeship team for later use
  }
  ...
  async VerifyOTP() {
    if (this.OTP.length > 0) {
      if ((this.OTP == GlobalConstants.DefaultOTP) || (this.OTP == this.GeneratedOTP)) {
        var id = this.ApprenticeshipTeamID;
        try {
  ...
  DownloadPdf(FileName: string): void {
    ...
    this.http.get(fileUrl, { responseType: 'blob' }).subscribe((blob: any) => {
      ...
      downloadLink.download = "ApprenticeshipDutyOrder.pdf"; // Set the desired file name
      downloadLink.click();
      ...
      async (result: any) => {
        if (result.isConfirmed) {
          try {
            ...
            this.loaderService.requestStarted();
            await this.itiApprenticeshipService.RequestApprove(DeplomentId)
              .then((data: any) => {
                data = JSON.parse(JSON.stringify(data));
