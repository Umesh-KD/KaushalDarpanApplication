...
export class VerifyStudentPhoneComponent {
  ...
  async GetDateConfig() {
    var data = {
      ...
      Key: "IMC ALLOTMENT CATEGORY,IMC ALLOTMENT ALL",
  ...
  async UpdateMobileNo() {
    if (this.NewMobileNo.length === 10) {
      ...
      this.request.MobileNo = this.NewMobileNo;
  ...
  async TradeWithAllot(content: any, CollegeTradeID: number, SeatMetrixId: number, AllotedCategory: string, SeatMetrixColumn: string, OpenCategory: string) {
    ...
    this.request.OpenCategory = OpenCategory;
