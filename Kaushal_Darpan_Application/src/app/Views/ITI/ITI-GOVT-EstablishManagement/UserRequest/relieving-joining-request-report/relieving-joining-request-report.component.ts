...
@Component({
  selector: 'app-relieving-joining-request-report',
  standalone: false,
  templateUrl: './relieving-joining-request-report.component.html',
  styleUrl: './relieving-joining-request-report.component.css'
})
export class RelievingJoiningRequestReportComponent {
  groupForm!: FormGroup;
  ...
  async ngOnInit() {
    ...
    if(this.RequestStatus){
      this.searchRequest.RequestStatus = this.RequestStatus
    }
    await this.GetRelievingJoiningRequestReportData();
  }
  ...
  async GetTransferStatusList() {
    try {
      await this.commonMasterService.GetCommonMasterData('TransferStatus')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.TransferStatusList = data['Data'];
        }, error => console.error(error));
    }
    catch (Ex) {
      console.log(Ex);
  ...
  async GetRelievingJoiningRequestReportData() {
    try {
      ...
      this.loaderService.requestStarted();
      await this.userRequestService.GetRelievingJoiningRequestReportData(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
