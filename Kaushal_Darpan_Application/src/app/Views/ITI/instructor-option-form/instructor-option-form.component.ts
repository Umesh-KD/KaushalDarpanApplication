...
@Component({
  selector: 'app-instructor-option-form',
  standalone: false,
  templateUrl: './instructor-option-form.component.html',
  styleUrl: './instructor-option-form.component.css'
})
export class InstructorOptionFormComponent {
  @Input() uid: number = 0;
...
/*    this.QualificationDataById()*/
  ...
  async PriorityChange(row: any, Type: string) {
    row.Type = Type

    try {
      await this.instructorservice.PriorityChange(row)
        .then(async (data: any) => {
          if (data.State == EnumStatus.Success) {
            this.toastr.success(data.Message)
            this.GetVerifiationList();
