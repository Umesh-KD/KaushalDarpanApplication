...
@Component({
  selector: 'app-scvt-center-allocation',
  standalone: false,
  templateUrl: './scvt-center-allocation.component.html',
  styleUrl: './scvt-center-allocation.component.css'
})
export class ScvtCenterAllocationComponent {
  ...
  async ddlInstitute_Change() {
    try {
      ...
      debugger
      //Center ID
      await this.centerAllocationService.GetscvtInstituteByCenterID(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
    ...
    try {
      ...
      await this.centerAllocationService.GetAllData(this.searchRequest)
        .then((data: any) => {
          ...
          console.log("main Data ===>",this.CenterMasterList);
        }, error => console.error(error));
    } catch (Ex) {
      console.log(Ex);
    } finally {
...
/*        const centersData = this.CenterMasterList.filter(f => f.Marked == 1).flatMap((center: any) => {*/
        ...
        debugger
        var centersData = {
          CenterID: this.searchRequest.CenterID,
        ...
        await this.centerAllocationService.Savescvtdata(centersData)
          .then((data: any) => {
            data = JSON.parse(JSON.stringify(data));
  ...
  onItemSelect(item: any, centerID: number) {
    const exists = this.SelectedInstituteList.some(x => x.ID === item.ID);
    if (!exists) {
  ...
  onDeSelect(item: any, centerID: number) {
    ...
    this.SelectedInstituteList = this.SelectedInstituteList.filter((i: any) => i.ID !== item.ID);
  ...
  exportToExcelCenterDetail() {
    const unwantedColumns = [
      ...
      'ModifyDate', 'IPAddress', 'CenterID', 'EndTermID', 'InstituteNames','InstituteIDs'
    ];

    const filteredData = this.CenterMasterList.map(
    ...
    XLSX.writeFile(wb, `Center_Allocation_download_Report_${dateStr}.xlsx`);
  }
