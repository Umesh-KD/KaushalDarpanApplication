...
import { ItiGetPassFailResultDataModel, ItiGetResultDataModel, ItiGetStudentRevisedResultDataModel } from '../../../../Models/ITI/ITI_ResultModel';
...
@Component({
  selector: 'app-iti-studentdetails-revised-result',
  standalone: false,
  templateUrl: './iti-studentdetails-revised-result.component.html',
  styleUrl: './iti-studentdetails-revised-result.component.css'
})
export class ITIStudentDetailsRevisedResultComponent {
  ...
  requestStudentRevisedModel: ItiGetStudentRevisedResultDataModel = new ItiGetStudentRevisedResultDataModel();
  ...
  async ngOnInit() {
    ...
    this.requestStudentRevisedModel.UserID = Number(this.sSOLoginDataModel.UserID);
    this.requestPassFailModel.EndTermID = Number(this.sSOLoginDataModel.EndTermID);
    this.requestStudentRevisedModel.EndTermID = Number(this.sSOLoginDataModel.EndTermID);
    this.requestPassFailModel.FinancialYearID = Number(this.sSOLoginDataModel.FinancialYearID);
    this.requestStudentRevisedModel.FinancialYearID = Number(this.sSOLoginDataModel.FinancialYearID);
    ...
    if (this.sSOLoginDataModel.RoleID != EnumRole.ITI_ResultAdmin) {    
      ...
      this.requestStudentRevisedModel.InstituteId = Number(this.sSOLoginDataModel.InstituteID);
    }
  ...
  //            this.GetStudentDetailsReviseResult();
  //          } else {
  ...
  async GetITICollegeStudent_Marksheet() {
    ...
    try {
      ...
      this.MarksheetSearch.TradeScheme = 2;
  ...
  async DownloadSCVTCertificate(row: any) {
    try {

      this.loaderService.requestStarted();
      const request = new ITIStateTradeCertificateSearchModel();
      request.EnrollmentNo = row.enrollment;
      ...
      await this.ReportServices.ITIStateTradeCertificateReport(request)
        .then((data: any) => {
          ...
          if (data && data.Data) {
            ...
            const link = document.createElement('a');
            link.href = blobUrl;
            link.download = row.enrollment + '_certificate.pdf';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(blobUrl);
  ...
  async onEditInfo(model: any, row: any) {
    try {
      await this.searchUpdateData();
  ...
  async searchUpdateData() {
    ...
    if (res && res.Data && res.Data.length > 0) {
      ...
      await this.GetStudentDetailsReviseResult();
