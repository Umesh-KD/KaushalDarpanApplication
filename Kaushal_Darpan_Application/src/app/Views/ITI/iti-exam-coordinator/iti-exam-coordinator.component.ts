...
@Component({
  selector: 'app-iti-exam-coordinator',
  standalone: false,
  templateUrl: './iti-exam-coordinator.component.html',
  styleUrl: './iti-exam-coordinator.component.css'
})
export class ItiExamCoordinatorComponent {
  ...
  async GetCenterMasterList() {
    ...
    try {
      ...
      await this.centerAllocationService.GetExamCoordinatorData(this.searchRequest)
        .then((data: any) => {
  ...
  downloadPDF(type: any) {
    ...
    this.searchRequest.DistrictID = this.sSOLoginDataModel.DistrictID
    try {
      this.centerAllocationService.DownloadExamCoordinate(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  openOTP() {
    debugger
    this.childComponent.MobileNo = this.sSOLoginDataModel.Mobileno ?? 0
