...
@Component({
  selector: 'app-inventory-item-report',
  standalone: false,
  templateUrl: './inventory-item-report.component.html',
  styleUrl: './inventory-item-report.component.css'
})
export class InventoryItemReportComponent {
  ...
  async ngOnInit() {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      this.Searchrequest.ConditionID = params['conditionID'] ?? 0;
      this.Searchrequest.IsAuction = params['isAuction'] ?? 0;
  ...
  async GetAllData() {
    ...
    try {
      ...
      await this.itiInventoryService.GetAllItemsMasterReport(this.Searchrequest)
        .then((data: any) => {
        ...
        if (result.isConfirmed) {
          try {
            ...
            try {
              ...
              debugger
              await this.itiInventoryService.UpdateStatusItemsData(this.request)
                .then((data: any) => {
                  this.State = data['State'];
                  this.Message = data['Message'];
                  this.ErrorMessage = data['ErrorMessage'];
