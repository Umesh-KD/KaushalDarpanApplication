...
@Component({
  selector: 'app-instructor-selectchoice',
  standalone: false,
  templateUrl: './instructor-selectchoice.component.html',
  styleUrl: './instructor-selectchoice.component.css'
})
export class InstructorSelectchoiceComponent {
  @Input() uid: number = 0;
  ...
  constructor(
    ...
    private instructorservice: ITI_InstructorService,
  ...
  async AddChoice() {
    ...
    if (this.CheckInstitute) {
      ...
      this.formData.TradeName = this.ItiTradeListAll.filter((x: any) => x.Id == this.formData.TradeID)[0]['TradeName'];
  ...
  async PriorityChange(row: any) {


    try {
      await this.instructorservice.Onfinaljoin(row)
        .then(async (data: any) => {
          if (data.State == EnumStatus.Success) {
            this.swat.Success(data.Message)
  ...
  async GetInstituteListDDL() {
    try {
      ...
      this.loaderService.requestStarted();
      await this.commonFunctionService.GetITIOptionFormData(this.tradeSearchRequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
  ...
  async OnFinalSubmit() {
    ...
    this.swat.Confirmation(
      "Are you sure you want to submit? Once submitted, you cannot change your choices.",
      async () => {
        ...
        try {
          ...
          console.log(ex);
          this.swat.Error("Something went wrong.");
