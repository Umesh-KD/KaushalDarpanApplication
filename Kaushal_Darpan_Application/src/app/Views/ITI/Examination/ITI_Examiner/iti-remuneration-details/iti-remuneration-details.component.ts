...
@Component({
  selector: 'app-iti-remuneration-details',
  templateUrl: './iti-remuneration-details.component.html',
  styleUrl: './iti-remuneration-details.component.css',
  standalone: false
})
export class ItiRemunerationDetailsComponent {
 ...
 // examinerId: number | null = null;
  ...
  async GenerateAndViewPdf(AppointExaminerID: number, adminstatus: number) {
    try {
      debugger;
          ...
          if (contentType === 'application/pdf') {//1.
            ...
            if (contentDisposition) {
              ...
              if (match && match[1]) {
                ...
                const idMatch = fileName.match(/^RemunerationExaminer(\d+)_/);
