...
@Component({
  selector: 'app-iti-student-enrollment',
  templateUrl: './iti-student-enrollment.component.html',
  styleUrl: './iti-student-enrollment.component.css',
  standalone: false
})
export class ItiStudentEnrollmentComponent
{
  ...
  async GetMasterData() {
    try {
      ...
      await this.commonMasterService.ItiGetStudentStatusByRole(this.sSOLoginDataModel.RoleID, 1)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
      ...
      await this.commonMasterService.ITIExamStudentStatus(this.sSOLoginDataModel.RoleID, 1)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async GetPreExamStudent() {
    try {
      ...
      this.loaderService.requestStarted();
      await this.studentExaminationITIService.GetPreEnrollStudent(this.request)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
          //
    ...
    try {
      ...
      await this.commonMasterService.ITIViewStudentDetails(model)
        .then((data: any) => {
          ...
          console.log(this.StudentProfileDetailsData, 'profiledata')
  ...
  async GetPreExam_StudentMaster(StudentID: number) {
    ...
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.ITIPreExam_StudentMaster(StudentID, this.request.StudentFilterStatusId, DepartmentID, Eng_NonEng, EndTermID)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async SaveData_EditStudentDetails() {
    ...
    if (this.IsVerified && this.sSOLoginDataModel.RoleID == EnumRole.ITIPrincipal && this.requestStudent.status == enumExamStudentStatus.SelectedForEnrollment) {
      this.requestStudent.status = enumExamStudentStatus.VerifiedForEnrollment// verified for enrollment(bter) 
    }
    else if (this.IsVerified && this.sSOLoginDataModel.RoleID == EnumRole.ITIPrincipal && this.requestStudent.status == enumExamStudentStatus.SelectedForExamination) {
  ...
  async SaveData_PreExam_UpdateEnrollmentNo() {
    ...
    this.loaderService.requestStarted();
    try {
      await this.studentExaminationITIService.SaveData_PreExam_UpdateEnrollmentNo(this.requestUpdateEnrollmentNo)
        .then(async (data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
  ...
  async SaveDataMarked() {
    ...
    if (this.status != enumExamStudentStatus.RejectatBTER) {
      ...
      if (this.request.StudentFilterStatusId == enumExamStudentStatus.SelectedForEnrollment && this.status != enumExamStudentStatus.VerifiedForEnrollment) {
        this.toastr.error("Please verify each student then choose 'Eligible For Enrollment' student!");
        return;
      }
    }
    ...
    try {
      ...
      if (this.status == enumExamStudentStatus.RejectatBTER) {
        if (this.request.StudentFilterStatusId != enumExamStudentStatus.RejectatBTER && this.sSOLoginDataModel.RoleID == EnumRole.DTETraing) {
      ...
      if (this.request.StudentFilterStatusId == enumExamStudentStatus.Addimited || this.request.StudentFilterStatusId == enumExamStudentStatus.New_Addimited) {
        if (this.status == enumExamStudentStatus.SelectedForEnrollment && this.sSOLoginDataModel.RoleID == EnumRole.ITIAdmin_SCVT || this.sSOLoginDataModel.RoleID == EnumRole.ITIAdmin_NCVT) {
          await this.SaveAdmittedStudentData();
        }
        else {
          this.toastr.error("Please choose 'Selected For Enrollment' Mark As with 'Admitted or New Admitted' status!");
        }
        return;
      }
      // eligible for enrollment
      if (this.request.StudentFilterStatusId == enumExamStudentStatus.VerifiedForEnrollment) {
        if (this.status == enumExamStudentStatus.EligibleForEnrollment && this.sSOLoginDataModel.RoleID == EnumRole.ITIPrincipal) {
          await this.SaveEligibleForEnrollment();
        }
        else {
          this.toastr.error("Please choose 'Eligible For Enrollment' Mark As with 'Verified For Enrollment' status!");
        }
        return;
      }
    ...
    this.Swal2.Confirmation("Are you sure to continue?", async (result: any) => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          await this.studentExaminationITIService.SaveAdmittedStudentData(request)
            .then(async (data: any) => {
              this.State = data['State'];
              this.Message = data['Message'];
              this.ErrorMessage = data['ErrorMessage'];
    ...
    this.Swal2.Confirmation("Are you sure to continue?", async (result: any) => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          // Call service to save student exam status
          await this.studentExaminationITIService.SaveEligibleForEnrollment(request)
            .then(async (data: any) => {
              this.State = data['State'];
              this.Message = data['Message'];
              this.ErrorMessage = data['ErrorMessage'];
  ...
  async GetPreExamStudentAnnexture() {
    try {
      ...
      await this.studentExaminationITIService.GetAnnextureListPreExamStudent(this.request)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
          //success
          if (data.State == EnumStatus.Success) {
