...
@Component({
  selector: 'app-iti-session-configuration',
  templateUrl: './iti-session-configuration.component.html',
  styleUrls: ['./iti-session-configuration.component.css'],
  standalone: false
})
export class ITISessionConfigurationComponent implements OnInit, OnDestroy {
  ...
  public allMonths: { id: number, name: string }[] = [];
  public OTP: string = '';
  ...
  async ngOnInit() {
    ...
    this.requestData.Month = "0";
    this.requestData.FYName = "0";
  ...
  chnageYear(selectedYear: number) {

    const currentMonth: number = new Date().getMonth() + 1; // JavaScript months are 0-indexed
    this.allMonths = [
      { id: 1, name: 'January' },
      { id: 2, name: 'February' },
      { id: 3, name: 'March' },
    ...
    if (selectedYear === currentYear) {
      this.monthList = this.allMonths.filter(m => m.id > currentMonth);
    } else {
      this.monthList = [...this.allMonths];
    }
  }
  ...
  async GetSesionDDL() {
    ...
    try {
      ...
      await this.commonMasterService.ExamSessionConfiguration(this.requestData)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async SaveData() {
    ...
    try {
      ...
      var th = this;
      this.requestData.FinancialYearName = this.requestData.FYName;// this.TypeMasterDDL.find(function (x) { return x.EndTerm = th.request.EndTermName })[0].FYName;
      this.requestData.EndTermName = ((this.allMonths.find(m => m.id === 2)?.name.substring(0, 4)) || '') + '-' + this.requestData.FYName.substring(0, 4);// this.request.EndTermName;
      //this.requestData.Month = this.request.Month;
      this.requestData.CourseTypeID = this.request.CourseTypeID;
      this.requestData.CreatedBy = this.sSOLoginDataModel.UserID;
      this.requestData.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      this.requestData.Action = "ADD";

      if (this.requestData.ExamType == '') {
  ...
  onToggleChange(event: Event, row: any) {
    ...
    this.Swal2.Confirmation(`Are you sure you want to ${newStatus ? 'Active' : 'In Active'} this session term?`, async (result: any) => {
      if (result.isConfirmed) {
        await this.ActiveInActive(row.EndTermID, newStatus);
  ...
  async ActiveInActive(FinancialYearID: number, ActiveStatus: boolean) {
    try {
      ...
      const data = await this.commonMasterService.ExamSessionConfiguration(this.requestData);
    ...
    if (this.OTP.length > 0) {
      if ((this.OTP == GlobalConstants.DefaultOTP) || (this.OTP == this.GeneratedOTP)) {
        try {
          ...
          var th = this;
          this.requestData.FinancialYearName = this.requestData.FYName;// this.TypeMasterDDL.find(function (x) { return x.EndTerm = th.request.EndTermName })[0].FYName;
          this.requestData.EndTermName = ((this.allMonths.find(m => m.id === parseInt(th.requestData.Month))?.name.substring(0, 3)) || '') + '-' + this.requestData.FYName.substring(0, 4);// this.request.EndTermName;
          this.requestData.CourseTypeID = this.request.CourseTypeID;
          this.requestData.CreatedBy = this.sSOLoginDataModel.UserID;
          this.requestData.DepartmentID = this.sSOLoginDataModel.DepartmentID;
          this.requestData.Action = "ADD";
  ...
  async openModalGenerateOTP(content: any, item: DateConfigurationModel) {
    ...
    else if (this.requestData.FYName == "0" || this.requestData.FYName == "")
    {
      this.toastr.error('Please select Session');
    }
    else if (this.requestData.Month == "0" || this.requestData.Month == "")
    {
      this.toastr.error('Please select Session Month');
    }
    else
    {
      this.OTP = '';
