...
import { ItiInvigilatorDatAModel } from '../../../../Models/ITI/ItiInvigilatorDataModel';
import { ITIInvigilatorService } from '../../../../Services/ITI/ITIInvigilator/itiinvigilator.service';
@Component({
  selector: 'app-scvt-exam-attendence',
  standalone: false,
  templateUrl: './scvt-exam-attendence.component.html',
  styleUrl: './scvt-exam-attendence.component.css'
})

export class ScvtExamAttendenceComponent {
  public searchRequest = new ItiInvigilatorDatAModel();
  _EnumRole = EnumRole
  ...
  constructor(
    private TheoryMarksService: ITIInvigilatorService,
  ...
  async GetTheoryMarksDetailList() {
    try {
      ...
      await this.TheoryMarksService.GetTheoryStudentsByRollRangeAsync(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));  
  ...
  ResetControl() {
    this.searchRequest = new ItiInvigilatorDatAModel();
  ...
  async SaveData() {
    ...
    try {
     ...
     const Data= this.TheoryMarksRptDataList.filter((x:any)=>x.Marked==true)
      ...
      await this.TheoryMarksService.SaveIsPresentData(Data)
        .then((data: any) => {
...
/*      this.sSOLoginDataModel.Mobileno = "8905268611";*/
  ...
  selectInTableSingleCheckbox(isSelected: boolean, item: any) {
    const data = this.TheoryMarksRptDataList.filter((x: any) => x.StudentExamPaperMarksID == item.StudentExamPaperMarksID);
    data.forEach((x:any) => {
      x.Marked = isSelected;
    });
    //select all(toggle)
