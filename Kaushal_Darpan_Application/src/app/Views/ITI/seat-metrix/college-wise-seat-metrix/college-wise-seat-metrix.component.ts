...
@Component({
  selector: 'app-college-wise-seat-metrix',
  templateUrl: './college-wise-seat-metrix.component.html',
  styleUrl: './college-wise-seat-metrix.component.css',
  standalone: false
})
export class CollegeWiseSeatMetrixComponent {
  ...
  async GetTradeAndColleges(i: any) {
    ...
    try {
      ...
      this.searchRequest.Action = 'COLLEGE-SEAT-MATRIX'
      this.searchRequest.CollegeID = this.SSOLoginDataModel.InstituteID;
      ...
      await this.ITICollegeTradeService.GetTradeAndColleges(this.searchRequest)
        .then((data: any) => {
          ...
          this.IsIMC = this.ListITICollegeTrade[0]?.IsImc==1;
        ...
        .then((data: any) => {
          ...
          if (data.State === EnumStatus.Success) {
            ...
            const unwantedColumns = [
              "TradeSchemeId", "SeatNotAvailable", "TotalRecords", "CollegeTradeId", "CollegeId", "TradeId", "IsPublish",	"MinPercentageInMath","MinPercentageInScience",	"OnlyForWomen",
              "OriginalSeat",	"Fee",	"ImcFee",	"IsImc"
            ];

            if (!this.IsIMC) {       
              unwantedColumns.push("IMC_SC");
              unwantedColumns.push("IMC_ST");
              unwantedColumns.push("IMC_OBC");
              unwantedColumns.push("IMC_GEN");
            }
  ...
  async exportPDFData() {
    ...
    try {
      ...
      this.loaderService.requestStarted();
      await this.ITICollegeTradeService.DownloadCollegeSeatMatrix(this.searchRequest)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data && data.Data) {
