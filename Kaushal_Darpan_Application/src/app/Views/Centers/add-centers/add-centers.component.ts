...
export class AddCentersComponent {
  ...
  async GetByID(id: number) {
    try {
      ...
      await this.centerService.GetByID(id)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  async saveData() {
    ...
    if (this.request.Capacity == 0) {
      this.toastr.warning("Capacity cannotb be zero !")
    }
    try {
      ...
      await this.centerService.SaveData(this.request)
        .then((data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
          ...
          if (this.State == EnumStatus.Success) {
            ...
            this.ResetControl();
            this.router.navigate(['/CenterCreate']);
  ...
  async ResetControl() {
    ...
    this.request = new CenterMasterDataModels();
