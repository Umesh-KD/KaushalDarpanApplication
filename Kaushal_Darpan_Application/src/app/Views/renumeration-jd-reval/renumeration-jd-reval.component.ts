...
import { RenumerationJdRevalService } from '../../Services/renumeration-jd-reval/renumeration-jd-reval.service';

@Component({
  selector: 'app-renumeration-jd-reval',
  standalone: false,
  templateUrl: './renumeration-jd-reval.component.html',
  styleUrl: './renumeration-jd-reval.component.css'
})
export class RenumerationJdRevalComponent {
  public Message: any = [];
  ...
  constructor(private commonMasterService: CommonFunctionService,
    ...
    private renumerationJdRevalService: RenumerationJdRevalService
  ) {
  }
  ...
  async GetAllData() {
    try {
      ...
      //call
      await this.renumerationJdRevalService.GetAllData(this.RenumerationJDRequest)
        .then(async (data: any) => {
  ...
  async SaveDataApprovedAndSendToAccounts() {
    try {
      ...
      //call
      await this.renumerationJdRevalService.SaveDataApprovedAndSendToAccounts(this.RenumerationJDSave)
        .then(async (data: any) => {
          if (data.State == EnumStatus.Error) {
            this.toastr.error(data.Message);
            console.log(data.ErrorMessage);
