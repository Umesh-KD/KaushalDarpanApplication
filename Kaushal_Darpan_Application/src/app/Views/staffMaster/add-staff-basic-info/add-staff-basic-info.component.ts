...
import { AddStaffBasicDetailDataModel, StaffDetailsDataModel, StaffMasterSearchModel, StaffHostelListModel } from '../../../Models/StaffMasterDataModel';
...
@Component({
  selector: 'app-add-staff-basic-info',
  standalone: false,
  
  templateUrl: './add-staff-basic-info.component.html',
  styleUrl: './add-staff-basic-info.component.css'
})
export class AddStaffBasicInfoComponent implements OnInit {
  ...
  constructor(private commonMasterService: CommonFunctionService, private Staffservice: StaffMasterService, private toastr: ToastrService, private loaderService: LoaderService, private formBuilder: FormBuilder, private activatedRoute: ActivatedRoute, private routers: Router, private modalService: NgbModal,private Swal2: SweetAlert2,) {
  ...
  onItemSelect(item: any, centerID: number) {
    ...
    if (!this.formData.HostelIDs.some(hostel => hostel.ID === item.ID)) {
      const selectedHostel = new StaffHostelListModel();
