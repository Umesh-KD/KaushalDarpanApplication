...
import { StaffDetailsDataModel, StaffSubjectList, Staff_EduQualificationDetailsModel } from '../../../Models/StaffMasterDataModel';
...
@Component({
    ...
    styleUrls: ['./add-staff-master.component.css'],
    standalone: false
})
export class AddStaffMasterComponent implements OnInit {
  ...
  EducationalQualificationFormData = new Staff_EduQualificationDetailsModel();
...
/*          await this.ddlStream_Change();*/
  ...
  ResetControls() {
    ...
    this.staffDetailsFormData = new StaffDetailsDataModel();
      ...
      async (result: any) => {
        ...
        if (result.isConfirmed) {
          try {
            this.loaderService.requestStarted();
            this.staffDetailsFormData.ModifyBy = this.sSOLoginDataModel.UserID
            await this.staffMasterService.ChangeWorkingInstitute(this.staffDetailsFormData).then((data: any) => {
              data = JSON.parse(JSON.stringify(data));
              console.log(data);
              if (data.State === EnumStatus.Success){
                this.toastr.success(data.Message);
  ...
  async GetCurrentWorkingInstitute_ByID() {
   try {
     ...
     this.loaderService.requestStarted();
     await this.staffMasterService.GetCurrentWorkingInstitute_ByID(this.searchRequest)
       .then((data: any) => {
         data = JSON.parse(JSON.stringify(data));
         console.log("data", data)
  ...
  onPanInput(event: any): void {
    ...
    const formattedValue = input.value.toUpperCase().slice(0, 10); // max 10 chars
