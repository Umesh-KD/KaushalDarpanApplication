...
@Component({
  selector: 'app-objection-window',
  standalone: false,
  
  templateUrl: './objection-window.component.html',
  styleUrl: './objection-window.component.css'
})
export class objectionwindowComponent {
  ...
  async GetAllGenerateHostelWardenStudentMeritlist() {
    debugger
      try {
        ...
        this.Searchrequest.Action = "getall_Obj";
  ...
  async objectionwindowSavedata() {
    ...
    try {
      this.Swal2.Confirmation(
        `Are you sure you want to save objection window merit list for ${selectedCount} selected student(s)?`,
        async (result: any) => {
          if (result.isConfirmed) {
            this.isSubmitted = true;
            this.loaderService.requestStarted();

            // Set ModifyBy only on selected students
            selected.forEach((item: any) => {
              item.ModifyBy = this.sSOLoginDataModel.UserID;
            });

            await this.studentRequestService.GetAllFinalCorrectionPublishHostelStudentMeritlist(selected)
              .then(async (data: any) => {
                this.State = data['State'];
                this.Message = data['Message'];
                this.ErrorMessage = data['ErrorMessage'];
              ...
              .catch((error: any) => {
                console.error("Save Objection Window Error:", error);
      ...
      console.log("Save Objection Window Exception:", ex);
  ...
  //    this.Swal2.Confirmation("Are you sure  want to Save Objection Window Merit List?", async (result: any) => {
  //      if (result.isConfirmed) {
  //        this.isSubmitted = true;
  //        this.loaderService.requestStarted();
  //        this.StudentReqListList.forEach((item: any) => {
  ...
  //        await this.studentRequestService.GetAllFinalCorrectionPublishHostelStudentMeritlist(this.StudentReqListList)
  //          .then(async (data: any) => {
  //            this.State = data['State'];
  //            this.Message = data['Message'];
  //            this.ErrorMessage = data['ErrorMessage'];
