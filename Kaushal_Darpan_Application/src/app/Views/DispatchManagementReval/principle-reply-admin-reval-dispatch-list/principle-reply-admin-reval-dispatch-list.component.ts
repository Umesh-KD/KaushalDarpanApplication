...
@Component({
  selector: 'app-principle-reply-admin-reval-dispatch-list',
  standalone: false,
  templateUrl: './principle-reply-admin-reval-dispatch-list.component.html',
  styleUrl: './principle-reply-admin-reval-dispatch-list.component.css'
})
export class PrincipleReplyAdminRevalDispatchListComponent {
    ...
    try {
      ...
      await this.commonMasterService.GetddlCenterDownloadOrReceived(Type)
        .then((data: any) => {
          ...
          this.StatusTypelist = this.StatusTypelist.filter((item: any) => item.ID == this._DispatchDDlValue.PrincipalVerifiedByToAdmin);
  ...
  async VerifiedtoPrincipalToAdmin() {
    try {
  ...
  async CheckOtpAfterSave() {
    ...
    Selected.forEach((e: any) => { e.Status = this._DispatchDDlValue.PrincipalVerifiedByToAdmin, e.ModifyBy = this.sSOLoginDataModel.UserID })
    this.loaderService.requestStarted();
   
    await this.DispatchRevalService.OnVerifiedtoPrincipalToAdmin(Selected).then(
      (data: any) => {
