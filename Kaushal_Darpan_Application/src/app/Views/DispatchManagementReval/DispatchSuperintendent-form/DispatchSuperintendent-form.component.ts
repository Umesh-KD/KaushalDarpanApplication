...
import { DispatchFormDataModel, BundelDataModel, DispatchSearchModel, CompanyDispatchMasterSearchModel } from '../../../Models/DispatchFormDataModel';
...
@Component({
  selector: 'app-DispatchSuperintendent-form',
  templateUrl: './DispatchSuperintendent-form.component.html',
  styleUrls: ['./DispatchSuperintendent-form.component.css'],
  standalone: false
})
export class DispatchSuperintendentFormComponent implements OnInit {
  ...
  async GetCompanyNameDDL() {
    ...
    try {
      ...
      await this.commonMasterService.GetDDLCompanyName(this.Searchrequest.DepartmentID, this.Searchrequest.CourseTypeID, this.Searchrequest.EndTermID)
        .then((data: any) => {
  ...
  //async CompanyWiseGetData() {
  //  
  //  try {
     
  //    this.SearchCompanyDispatch.CompanyID = Number(this.request.CompanyName);
  //    this.loaderService.requestStarted();
  //    await this.dispatchService.GetByIdCompanyDispatch(this.SearchCompanyDispatch.CompanyID)
  //      .then((data: any) => {
  //        data = JSON.parse(JSON.stringify(data));
  //        this.State = data['State'];
  //        this.Message = data['Message'];
  //        this.ErrorMessage = data['ErrorMessage'];
  //        this.CompanyWiseDispatchList = data['Data'];
        
  //        this.request.SupplierName = this.CompanyWiseDispatchList[0]["SupplierName"];
  //        this.request.SupplierVehicleNo = this.CompanyWiseDispatchList[0]["SupplierVehicleNo"];
  //        this.request.SupplierMobileNo = this.CompanyWiseDispatchList[0]["SupplierMobileNo"];
  //        console.log('company wise data',this.CompanyWiseDispatchList)
  //      }, error => console.error(error));
  //  }
  //  catch (Ex) {
  //    console.log(Ex);
  ...
  // toggleAllSelection() {
  //   
  //   this.request.BundelDataModel.forEach(item => {
  ...
  // checkIfAllSelected() {
  //   
  //   const relevantItems = this.request.BundelDataModel.filter(item => item.DispatchID === 0);
  //   this.isAllSelected = relevantItems.length > 0 && relevantItems.every(item => item.selected);
  // }
    ...
    try {
      ...
      await this.dispatchService.GetAllData(this.Searchrequest)
        .then((data: any) => {
          ...
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
          this.DispatchMasterList = data['Data'];
          if (this.DispatchMasterList.length < 1) {
            this.IsDisplay = false
          }
          console.log(this.DispatchMasterList)
        }, error => console.error(error));
    }
    catch (Ex) {
      console.log(Ex);
  ...
  async clickDateChange() {
    ...
    /*  await this.searchBundelDataByDate(this.SearchBundelrequest.SearchDate);*/
  ...
  async searchBundelDataByDate() {
    ...
    try {
      ...
      this.SearchBundelrequest.UserID = this.sSOLoginDataModel.UserID;
      ...
      //if (this.request.BundelDataModel.length > 0) {
      //  this.CheckDispatchId = this.request.BundelDataModel[0]["DispatchID"];
      //  this.CheckDispatchDate = this.request.BundelDataModel[0]["DispatchDate"];
      //  this.CheckDispatchChallanNo = this.request.BundelDataModel[0]["ChallanNo"];
      //  this.CheckExamDate == this.request.BundelDataModel[0]["ExamDate"];
      //} else {
      //  this.CheckDispatchId = 0;
      //  this.CheckDispatchDate = '';
      //  this.CheckDispatchChallanNo = '';
      //}
    ...
    try {
      ...
      await this.dispatchService.GetByIdDispatchMaster(this.DispatchID)
        ...
        .then((data: any) => {
          ...
          /*const rawDate = this.request.BundelDataModel[0]["ExamDate"];*/
          ...
          //this.SearchBundelrequest.SearchDate = `${day}-${month}-${year}`; // Output: '13-04-2025'
          ...
          this.isUpdateMode = true;
          this.IsDisplay = true;
          const dateFields: (keyof DispatchFormDataModel)[] = [
        ...
        await this.dispatchService.SaveData(this.request)
          .then((data: any) => {
            ...
            if (this.State == EnumStatus.Success) {
              ...
              this.router.navigate(['/DispatchSuperintendentDetailsList'])
