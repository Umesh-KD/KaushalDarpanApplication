...
import { RevalDispatchGroupDataModel, DispatchGroupSearchModel, InstituteGroupDetail, RevalDispatchGroupcodeModel } from '../../../Models/DispatchGroupDataModel';
...
@Component({
  selector: 'app-dispatch-to-institute-reval',
  standalone: false,
  templateUrl: './dispatch-to-institute-reval.component.html',
  styleUrl: './dispatch-to-institute-reval.component.css'
})
export class DispatchToInstituteRevalComponent {
...
/*  public MasterDDLList:any[]=[]*/
  ...
  public Principledetail : any[] = [];
  public request = new RevalDispatchGroupDataModel();
  public Searchrequest = new DispatchGroupSearchModel();
  public GroupDataModelSelect :  RevalDispatchGroupcodeModel[] = [];
...
/*  public Searchrequest = new BundelDataModel();*/
  ...
  async ngOnInit() {
    ...
    this.DispatchForm = this.formBuilder.group({
      ...
      txtSupplierName: [{ value: '', disabled: true }, Validators.required],
      txtSupplierVehicleNo: [{ value: '', disabled: true }, Validators.required],
      txtSupplierMobileNo: [
        { value: '', disabled: true },
  ...
  async CompanyWiseGetData(): Promise<void> {
    try {
      ...
      const response = await this.dispatchRevalService.GetByIdCompanyDispatch(this.SearchCompanyDispatch.CompanyID);
  ...
  async onInstituteChange() {
    ...
    this.request = new RevalDispatchGroupDataModel()
  ...
  async searchBundelDataByDate() {
    try {
      ...
      const data: any = await this.dispatchRevalService.GetGroupDataAllData(this.Searchrequest);
      debugger
...
/*      this.Principledetail.GroupDataModel = []*/
  ...
  async getCheckDateDispatch() {
    ...
    try {
      await this.dispatchRevalService.CheckDateDispatchSearch(this.SearchCheck).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
...
checkboxthView_checkboxchange(isChecked: boolean) {
  ...
  for (let item of this.Principledetail) {
  ...
  async SaveData() {
    ...
    const selectedItems = this.Principledetail.filter((item: any) => item.Selected === true);
    ...
    try {
      ...
      this.request.GroupDataModel = selectedItems;
      ...
      await this.dispatchRevalService.SaveDispatchGroup(this.request)
        .then((data: any) => {
          this.State = data['State'];
          this.Message = data['Message'];
          this.ErrorMessage = data['ErrorMessage'];
  ...
  async GetById() {
    try {
      ...
      await this.dispatchRevalService.GetRevalGroupdetailsId(this.DispatchID)
        ...
        .then((data: any) => {
          ...
          this.Principledetail = this.request.GroupDataModel;
          const dateFields: (keyof RevalDispatchGroupDataModel)[] = [
