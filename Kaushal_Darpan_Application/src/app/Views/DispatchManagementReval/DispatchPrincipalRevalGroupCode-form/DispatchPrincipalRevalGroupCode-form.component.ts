...
import { EnumCourseType, EnumDepartment, EnumLateralCourse, EnumStatus, EnumRevalDispatchDDlValue } from '../../../Common/GlobalConstants';
...
@Component({
  selector: 'app-DispatchPrincipalRevalGroupCode-form',
  templateUrl: './DispatchPrincipalRevalGroupCode-form.component.html',
  styleUrls: ['./DispatchPrincipalRevalGroupCode-form.component.css'],
  standalone: false
})
export class DispatchPrincipalRevalGroupCodeFormComponent implements OnInit {
 ...
 /* public GroupList: any[] = [];*/
  ...
  async GetDDLDispatchNo() {
    ...
    try {
      ...
      await this.commonMasterService.GetDDLRevalDispatchNo(this.sSOLoginDataModel.DepartmentID, this.sSOLoginDataModel.Eng_NonEng, this.sSOLoginDataModel.EndTermID)
        .then((data: any) => {
  ...
  async getgroupteacherData() {
    ...
    try {
      await this.dispatchRevalService.GetRevalDispatchGroupcodeDetailsCheck(this.Searchrequest).then((data: any) => {
        data = JSON.parse(JSON.stringify(data));
        ...
        this.IsHandOverToPrincipalByExaminer = this.GroupList.every((item: any) => item.ExaminerStatusID === this._DispatchDDlValue.ExaminerVerifiedByToPrincipal);
  ...
  async SaveData() {
    ...
    //} else {
      try {
        ...
        await this.dispatchRevalService.SaveRevalDispatchPrincipalGroupCodeData(this.request)
          .then((data: any) => {
            this.State = data['State'];
            this.Message = data['Message'];
            this.ErrorMessage = data['ErrorMessage'];
            ...
            if (this.State == EnumStatus.Success) {
              ...
              this.router.navigate(['/DispatchPrincipalRevalGroupCodeList']);
  ...
  async GetById() {
    ...
    try {
      ...
      await this.dispatchRevalService.GetRevalDispatchGroupcodeId(this.DPGCID)
