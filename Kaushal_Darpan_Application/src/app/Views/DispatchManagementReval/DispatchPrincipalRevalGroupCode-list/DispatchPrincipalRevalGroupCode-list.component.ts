...
@Component({
  selector: 'app-DispatchPrincipalRevalGroupCode-list',
  standalone: false,
  templateUrl: './DispatchPrincipalRevalGroupCode-list.component.html',
  styleUrl: './DispatchPrincipalRevalGroupCode-list.component.css'
})
export class DispatchPrincipalRevalGroupCodeListComponent {
  ...
  async getgroupData() {
    ...
    //      .filter((item: any) => item.StatusID === this.StatusID)
    //      .map((item: any) => {
    //        return {
    //          id: item.ID,
    //          groupName: `${item.Code} - ${item.Description}`,
    //          status: item.StatusID // mapped as "status"
    //        };
    //      })[0];

    //    if (firstItem) {
    //      this.GetStatusID = firstItem.status; // ✅ use "status", not "StatusID"
    //    }
    ...
    //}
    try {
      const data: any = await this.DispatchRevalService.GetRevalDispatchGroupcodeList(this.searchRequest);

      const groupData = data?.Data || [];
      this.DispatchPrincipalGroupCodeList1 = groupData;

      console.log(this.StatusID, "status");

        this.GetStatusID = this.DispatchPrincipalGroupCodeList1[0].StatusID;
      

      console.log("this.DispatchPrincipalGroupCodeList", this.DispatchPrincipalGroupCodeList1);

    } catch (error) {
      console.error("Error fetching dispatch group code list:", error);
    }
    ...
    try {
      ...
      await this.commonMasterService.GetddlCenterDownloadOrReceived(Type)
        .then((data: any) => {
          ...
          this.StatusTypelist = this.StatusTypelist.filter((item: any) => item.ID == this._DispatchDDlValue.PrincipalDespoistToAdmin || item.ID == this._DispatchDDlValue.AdminToPrincipalDespoistReply);
  ...
  async CheckOtpAfterSave() {
    ...
    for (const item of selectedItems) {
      this.UpdateStatusDispatchlist.push({
        ...
        Status: this._DispatchDDlValue.PrincipalDespoistToAdmin,
        Action: '',
