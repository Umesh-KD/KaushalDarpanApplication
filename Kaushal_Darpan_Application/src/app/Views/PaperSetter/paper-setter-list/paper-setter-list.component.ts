...
import { EnumPaperSetterStatus, EnumRole, EnumStatus, GlobalConstants } from '../../../Common/GlobalConstants';
...
@Component({
  selector: 'app-paper-setter-list',
  standalone: false,
  templateUrl: './paper-setter-list.component.html',
  styleUrl: './paper-setter-list.component.css'
})
export class PaperSetterListComponent implements OnInit {
  ...
  public staffReq = new VerifyPaperSetterDataModel();
  ...
  async DownloadExaminerForm(PaperSetterID: number) {
    try {
      this.loaderService.requestStarted();
      this.StaffID = this.AppointExaminer.PaperSetterID;
      await this.reportService.GetExaminersDetails(PaperSetterID, this.sSOLoginDataModel.DepartmentID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
  ...
  async GetStaffOrder(StaffID: number) {
    try {
      this.loaderService.requestStarted();
      this.staffReq.StaffID = StaffID;
      this.staffReq.PaperSetterID = this.PaperSetterID;

      await this.papersetterservice.GetStaffOrder(this.staffReq)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data.State == EnumStatus.Success) {
            this.DownloadFile(data.Data, 'file download');
  ...
  async DownloadOrder(FileName: any) {
    this.DownloadFile(FileName, 'file download');
  }
  ...
  selectInTableSingleCheckbox(isSelected: boolean, item: any) {
    const data = this.ExaminersList.filter(x => x.StaffID == item.StaffID);
    data.forEach(x => {
